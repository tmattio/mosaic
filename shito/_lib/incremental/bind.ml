...
let phys_same (type a b) (a : a) (b : b) = a == (Stdlib.Obj.magic b : a)
...
let invariant _invariant_a _invariant_b t =
  ...
  (assert (t.lhs_change.created_in == t.main.created_in);
   match t.lhs_change.kind with
   | Invalid -> ()
   | Bind_lhs_change t' -> assert (same t t')
   | _ -> assert false);
  
  (match t.rhs_scope with
   | Scope.Top -> assert false
   | Bind t' -> assert (same t t'));
  
  iter_nodes_created_on_rhs t ~f:(fun (T node) ->
    assert (node.created_in == t.rhs_scope);
