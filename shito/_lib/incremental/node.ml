module Uopt = Types.Uopt
module Uniform_array = Types.Uniform_array
...
type 'a t = 'a Types.Node.t =
  ...
  ; creation_backtrace : Types.Backtrace.t option
  }
...
let invariant (type a) (invariant_a : a -> unit) (t : a t) =
  assert ((needs_to_be_computed t) = (is_in_recompute_heap t));
  if is_necessary t
  then (
  ...
  Node_id.invariant t.id;
  Stabilization_num.invariant t.recomputed_at;
  
  if is_valid t && not (is_stale t) then assert (Uopt.is_some t.value_opt);
  (match t.value_opt with
   | None -> ()
   | Some v -> invariant_a v);
  
  Kind.invariant invariant_a t.kind;
  (match t.kind with
   | Expert e ->
     Expert.invariant_about_num_invalid_children e ~is_necessary:(is_necessary t)
   | _ -> ());
  
  Cutoff.invariant invariant_a t.cutoff;
  
  Stabilization_num.invariant t.changed_at;
  if Stabilization_num.is_some t.recomputed_at
  then assert (Stabilization_num.compare t.changed_at t.recomputed_at <= 0);
  
  assert (t.num_on_update_handlers =
          (List.length t.on_update_handlers
           + fold_observers t ~init:0 ~f:(fun n { on_update_handlers; _ } ->
             n + List.length on_update_handlers)));
  
  assert (t.num_parents >= 0);
  assert (t.num_parents <= 1 + Uniform_array.length t.parent1_and_beyond);
  
  for parent_index = 1 to Uniform_array.length t.parent1_and_beyond do
    assert ((parent_index < t.num_parents) = 
            (Uopt.is_some (Uniform_array.get t.parent1_and_beyond (parent_index - 1))))
  done;
  
  assert ((t.num_parents > 0) = (Uopt.is_some t.parent0));
  
  Scope.invariant t.created_in;
  
  if Scope.is_top t.created_in || not (is_valid t)
  then assert (Uopt.is_none t.next_node_in_same_scope);
  
  if is_necessary t then assert (t.height >= 0) else assert (t.height = -1);
  
  assert (t.height_in_recompute_heap >= -1);
  assert (t.height_in_recompute_heap <= t.height);
  
  if not (is_in_recompute_heap t)
  then assert (Uopt.is_none t.prev_in_recompute_heap);
  if Uopt.is_some t.prev_in_recompute_heap
  then (
    let (T prev) = Uopt.unsafe_value t.prev_in_recompute_heap in
    assert (packed_same (T t) (Uopt.unsafe_value prev.next_in_recompute_heap));
    assert (t.height_in_recompute_heap = prev.height_in_recompute_heap));
  
  if not (is_in_recompute_heap t)
  then assert (Uopt.is_none t.next_in_recompute_heap);
  if Uopt.is_some t.next_in_recompute_heap
  then (
    let (T next) = Uopt.unsafe_value t.next_in_recompute_heap in
    assert (packed_same (T t) (Uopt.unsafe_value next.prev_in_recompute_heap));
    assert (t.height_in_recompute_heap = next.height_in_recompute_heap));
  
  if t.height_in_adjust_heights_heap >= 0
  then assert (t.height_in_adjust_heights_heap < t.height);
  
  if not (is_in_adjust_heights_heap t)
  then assert (Uopt.is_none t.next_in_adjust_heights_heap)
  else if Uopt.is_some t.next_in_adjust_heights_heap
  then (
    let (T next) = Uopt.unsafe_value t.next_in_adjust_heights_heap in
    assert (is_in_adjust_heights_heap next);
    assert (t.height_in_adjust_heights_heap = next.height_in_adjust_heights_heap));
  
  (match t.old_value_opt with
   | None -> ()
   | Some v -> invariant_a v);
  ...
  (match t.kind with
   | Expert _ -> ()
   | _ ->
     assert (Array.length t.my_parent_index_in_child_at_index = initial_num_children t));
    ...
    iteri_children t ~f:(fun child_index (T child) ->
      assert (
        packed_same
          ...
          (get_parent
             child
             ~index:t.my_parent_index_in_child_at_index.(child_index))));
  
  if debug && not (is_necessary t)
  then Array.iter (fun x -> assert (x = -1)) t.my_parent_index_in_child_at_index;
  
  assert (Array.length t.my_child_index_in_parent_at_index = 
          Uniform_array.length t.parent1_and_beyond + 1);
  ...
  iteri_parents t ~f:(fun parent_index (T parent) ->
    assert (
      packed_same
        ...
        (Kind.slow_get_child
           parent.kind
           ~index:t.my_child_index_in_parent_at_index.(parent_index))));
  
  if debug && not (is_necessary t)
  then Array.iter (fun x -> assert (x = -1)) t.my_child_index_in_parent_at_index
;;
...
let value_exn t =
  ...
  else failwith "attempt to get value of an invalid node"
;;
