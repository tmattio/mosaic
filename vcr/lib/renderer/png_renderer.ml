...
type config = {
  ...
  cursor_style : [ `Block | `Underline | `Bar ];  (** Cursor style *)
  cursor_color : (int * int * int) option;  (** Custom cursor color (RGB) *)
}
...
let get_color theme color reversed is_fg =
  let actual_color = if reversed then (if is_fg then color else color) else color in
  match actual_color with
  | Ansi.Default -> if is_fg then theme.Gif_renderer.fg else theme.Gif_renderer.bg
  | Ansi.Black -> if not reversed then theme.Gif_renderer.black else theme.Gif_renderer.white
  ...
  | Ansi.White -> if not reversed then theme.Gif_renderer.white else theme.Gif_renderer.black
...
let capture_frame t =
  ...
  let (term_width, term_height) =
    ...
    | _ ->
        (cols * t.config.char_width, rows * t.config.char_height)
  in
  ...
  for row = 0 to rows - 1 do
    for col = 0 to cols - 1 do
      ...
      | Some cell ->
          ...
          (* Get colors *)
          let fg_color = get_color t.config.theme cell.style.fg cell.style.reversed true in
          let bg_color = get_color t.config.theme cell.style.bg cell.style.reversed false in
          ...
          (* Draw character *)
          render_char pixels full_width full_height x y cell.char fg_color bg_color 
  ...
  if Vte.is_cursor_visible t.vte && cur_row >= 0 && cur_row < rows && 
     cur_col >= 0 && cur_col < cols then (
    let x = t.config.padding + (cur_col * char_width) in
    let y = t.config.padding + (cur_row * char_height) in
    
    let cursor_color = match t.config.cursor_color with
      | Some color -> color
      | None -> 
          (* Invert the current background color *)
          let idx = (y * full_width) + x in
          if idx >= 0 && idx < Array.length pixels then
            let r, g, b = pixels.(idx) in
            (255 - r, 255 - g, 255 - b)
          else t.config.theme.fg
    in
    
    match t.config.cursor_style with
    | `Block ->
    ...
    | `Underline ->
        ...
        let underline_y = y + char_height - 2 in
        if underline_y >= 0 && underline_y < full_height then
          for dx = 0 to char_width - 1 do
            let px = x + dx in
            if px >= 0 && px < full_width then
              let idx = (underline_y * full_width) + px in
              pixels.(idx) <- cursor_color
          done
    | `Bar ->
        ...
        for dy = 0 to char_height - 1 do
          let py = y + dy in
          if x >= 0 && x < full_width && py >= 0 && py < full_height then
            let idx = (py * full_width) + x in
            pixels.(idx) <- cursor_color
        done
  );
...
let render t =
  ...
  | Some pixels ->
      ...
      let pixel_array = Array.init t.height (fun y ->
        Array.init t.width (fun x ->
          pixels.((y * t.width) + x)
        )
      ) in
