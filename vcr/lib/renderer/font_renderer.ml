      ...
      let italic =
        try
          Freetype.create
            ~font_path:
              (Filename.remove_extension path
              ^ "-Italic" ^ Filename.extension path)
            ~pixel_size:font_size
        with _ -> regular
      in
      let bold_italic =
        try
          Freetype.create
            ~font_path:
              (Filename.remove_extension path
              ^ "-BoldItalic" ^ Filename.extension path)
            ~pixel_size:font_size
        with _ -> bold
      in
...
let get_char_dimensions = function
  ...
  | Freetype _ -> (10, 20)
(* Default dimensions for Freetype fonts *)
...
let render_char pixels width height x y char fg_color _bg_color char_width
    char_height =
