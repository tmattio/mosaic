...
let rec rgb_of_color = function
  | Index n ->
      ...
      if n < 8 then (* 0–7  : simple colors *)
        ...
        let r = basic.(n land 0x1)
        and g = basic.((n asr 1) land 0x1)
        and b = basic.((n asr 2) land 0x1) in
        (r, g, b)
      else if n < 16 then rgb_of_color (Index (n - 8))
      else if n < 232 then (* color cube 6×6×6 *)
        let n = n - 16 in
        let step v = if v = 0 then 0 else 95 + (40 * v) in
        let r = step (n / 36)
        and g = step (n / 6 mod 6)
        and b = step (n mod 6) in
        (r, g, b)
      else (* greyscale ramp *)
        let g = 8 + (10 * (n - 232)) in
        (g, g, g)
  | RGB (r, g, b) -> (r, g, b)
  | _ ->
      (* default / hi‑colors not mapped here *)
      (0, 0, 0)
