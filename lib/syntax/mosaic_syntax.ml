...
type theme =
  ...
  | `Solarized_dark
  | `Solarized_light
  | `Custom of (string * Mosaic.Style.t) list ]
...
let find_style (theme_map : Style.t String_map.t) (scopes : string list) :
    Style.t =
    ...
    List.iter
      (fun entry ->
        ...
        | Some fg_color ->
            ...
            let style_attrs = ref [ Style.Fg color ] in
            (match font_style with
            | Some "italic" -> style_attrs := Style.Italic :: !style_attrs
            | Some "bold" -> style_attrs := Style.Bold :: !style_attrs
            | Some "underline" -> style_attrs := Style.Underline :: !style_attrs
            | _ -> ());

            let style = Style.of_list !style_attrs in
            List.iter
