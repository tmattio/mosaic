...
(** [measure ?width element] returns the (width, height) that the element would require
    when rendered. The optional width parameter constrains the measurement for
    width-aware elements like Flow and vertical Box layouts. Useful for dynamic 
    layout calculations. *)
