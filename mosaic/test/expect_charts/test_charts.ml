...
let%expect_test "bar chart - negative values with zero baseline" =
...
let%expect_test "line chart - with axis labels" =
  print_ui ~width:30 ~height:10
    (line ~width:(`Cells 30) ~height:(`Cells 10) ~show_axes:true
...
let%expect_test "bar chart - mixed positive and negative" =
...
let%expect_test "horizontal bar with negative values" =
  print_ui ~width:30 ~height:8
    (bar ~width:(`Cells 30) ~height:(`Cells 8) ~orientation:`Horizontal
       ~bar_width:1
       [
