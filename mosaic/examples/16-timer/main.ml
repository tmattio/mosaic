...
let update msg m = 
    ...
    | Tick dt ->
        if m.state = Running then
          ...
          if elapsed >= 1.0 then
            ...
            ({ m with time_remaining = new_time; state = new_state; elapsed_time = 0.0 }, Cmd.none)
          else
            ({ m with elapsed_time = elapsed }, Cmd.none)
        else
          (m, Cmd.none)
    ...
    | Set_time ->
        ...
        ({ m with time_remaining = total_seconds; initial_time = total_seconds; state = Idle; elapsed_time = 0.0 }, Cmd.none)
    | Quit -> (m, Cmd.quit)
    ...
    scroll_box ~scroll_y:true ~scroll_x:false
        ...
        [
            box ~align_items:Center ~justify_content:Center  
                ~size:{ width = pct 100; height = pct 100 }
                [
                    box ~flex_direction:Column ~align_items:Center ~gap:(gap 2) ~border:true ~padding:(padding 2) ~title:"Timer"   
                    [
                ...
                box ~border:true ~padding:(padding 1)
                    ~on_mouse:(fun ev ->
                        ...
                        | _ -> None)
                    [
                        text ~text_style:(Ansi.Style.make ~bold:true ()) "Set Time";
                    ];
                    ...
                    [
                        box ~border:true ~padding:(padding 1)
                            ~on_mouse:(fun ev ->
                                ...
                                | _ -> None)
                            [
                                text ~text_style:(Ansi.Style.make ~bold:true ~fg:Ansi.Color.green ()) "Start";
                            ];
                        box ~border:true ~padding:(padding 1)
                            ~on_mouse:(fun ev ->
                                match Mosaic_ui.Event.Mouse.kind ev with
                                ...
                                | _ -> None)
                            [
                                text ~text_style:(Ansi.Style.make ~bold:true ~fg:Ansi.Color.red ()) "Stop";
                            ];
                        box ~border:true ~padding:(padding 1)
                            ~on_mouse:(fun ev ->
                                match Mosaic_ui.Event.Mouse.kind ev with
                                ...
                                | _ -> None)
                            [
                                text ~text_style:(Ansi.Style.make ~bold:true ~fg:Ansi.Color.yellow ()) "Reset";
                            ];
                    ];
                
                text
                    ~text_style:(Ansi.Style.make ~dim:true ())
                    "Press Space to start/stop, R to reset, Q to quit";
                    ]
                ]
        ]
