...
module Starfield = struct
  ...
  let random_star width height =
    {
      x = Random.int width;
      y = Random.int height;
      brightness = Random.int 4;  (* More brightness levels *)
      twinkle_phase = Random.float (2.0 *. Float.pi);
      speed = 0.5 +. Random.float 2.0;  (* Variable twinkle speeds *)
    }
  ...
  let update_asteroid asteroid frame width height =
    if asteroid.active then
      ...
      if new_x > float_of_int width || new_y > float_of_int height 
         || new_x < -10.0 || new_y < -5.0 then
        { asteroid with active = false }
      else { asteroid with ax = new_x; ay = new_y }
    else if frame mod 30 = 0 && Random.int 100 < 15 then  (* 15% chance every 30 frames *)
      let start_side = Random.int 4 in
      let (sx, sy, speed_x, speed_y) = 
        match start_side with
        | 0 -> (* From left *)
            (0.0, Random.float (float_of_int height), 
             1.5 +. Random.float 2.0, Random.float 1.0 -. 0.5)
        | 1 -> (* From top *)
            (Random.float (float_of_int width), 0.0,
             Random.float 1.0 -. 0.5, 1.0 +. Random.float 1.5)
        | 2 -> (* From right *)
            (float_of_int width, Random.float (float_of_int height),
             -1.5 -. Random.float 2.0, Random.float 1.0 -. 0.5)
        | _ -> (* Diagonal from top-left *)
            (Random.float 20.0, 0.0,
             1.0 +. Random.float 1.5, 0.5 +. Random.float 1.0)
      in
      { 
        ax = sx; 
        ay = sy; 
  ...
  let render width height stars asteroids =
    ...
    List.iter (fun star ->
      if star.y >= 0 && star.y < height && star.x >= 0 && star.x < width then
        grid.(star.y).(star.x) <- Some (star_char star.brightness, Style.(fg (Index 252)))
    ) stars;
...
let chat_agent_app () =
  ...
  let stars, _, update_stars = use_state (Starfield.init_stars window_width window_height 3) in
  let asteroids, _, update_asteroids = use_state 
    (List.init 5 (fun _ -> 
      { Starfield.ax = 0.0; ay = 0.0; active = false; 
        trail_length = 5; speed_x = 0.0; speed_y = 0.0; char_type = 0 })) in
  ...
  use_tick (fun delta ->
    ...
    update_asteroids (fun current_asteroids ->
      List.map (fun a -> Starfield.update_asteroid a (frame + 1) window_width window_height) 
        current_asteroids));
  ...
  let starfield_grid = Starfield.render window_width window_height stars asteroids in
  let starfield_background =
    vbox ~gap:(`Cells 0)
      (Array.to_list (Array.mapi (fun _y row ->
        hbox ~gap:(`Cells 0)
          (Array.to_list (Array.map (fun cell ->
            match cell with
  ...
  let messages_view =
    if List.length messages = 0 then
      vbox ~gap:(`Cells 1)
        ...
        [
          text "# New session";
          text "/share to create a shareable link";
        ]
    else
      vbox ~gap:(`Cells 1)
        (List.rev messages
        |> List.map (fun msg ->
               box ~padding:(sides ~left:1 ~right:1 ())
...
let () =
  Random.self_init ();
  run ~alt_screen:true ~mouse:true ~fps:30 chat_agent_app ~debug:"mosaic-chat.log"