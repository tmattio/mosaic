...
let render t renderable grid ~delta:_ =
  ...
  let resolve_link idx =
    ...
    if Int32.equal raw Int32.minus_one then None
    else Text_buffer.link_at_index t.buffer (Int32.to_int raw)
  in
    ...
    if dest_y >= 0 && dest_y < buffer_height then
      let rec loop i column =
        if i >= line.length || column >= draw_width_limit then ()
          ...
          if Int32.to_int code = 10 then loop (i + 1) column
          else
            ...
            if dest_x >= 0 && dest_x < buffer_width then (
              ...
              in
              let is_tab = Int32.to_int code = 9 in
              if is_tab then (
                ...
                for off = 0 to draw_width - 1 do
                  let char_code =
                    if off = 0 then
                      ...
                      | Some cp -> Int32.of_int cp
                      | None -> Int32.of_int 32
                    else Int32.of_int 32
                  in
                  let char_fg =
                    if off = 0 then
