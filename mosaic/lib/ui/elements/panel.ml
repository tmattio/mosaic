...
let panel ?(box_style = Border.Rounded) ?title ?(title_align = `Center)
  ...
  let top_text =
    ...
    | Some "" | None -> None
    | Some t ->
        Some { Border.text = t; align = title_align; style = Some title_style }
  in
  ...
  let bottom_text =
    ...
    | Some s ->
        Some
          { Border.text = s; align = subtitle_align; style = Some title_style }
  in
  ...
  let title_min_width =
    match title with
    | Some t when t <> "" ->
        String.length t + 4 (* 2 for padding, 2 for border chars *)
    | _ -> 0
  in
  let subtitle_min_width =
    match subtitle with Some s when s <> "" -> String.length s + 4 | _ -> 0
  in
  ...
  let panel_content =
    E.box ~border ~border_style ~padding:actual_padding
      ?width:(Option.map (fun w -> `Cells w) width)
      ?height:(Option.map (fun h -> `Cells h) height)
      ~min_width:
        (if text_based_min_width > 0 then `Cells text_based_min_width else `Auto)
