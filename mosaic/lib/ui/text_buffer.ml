...
type style_snapshot = {
  ...
  links : (int32, int32_elt, c_layout) Array1.t;
}
...
let empty_cell () = { code = 0l; width = 0; base_style = Ansi.Style.default }
...
type t = {
  ...
  mutable tab_indicator : int32 option;
  ...
  mutable length : int;
  mutable chars : (int32, int32_elt, c_layout) Array1.t;
  ...
  mutable link_indices : (int32, int32_elt, c_layout) Array1.t;
...
let write_link t idx = function
  | None -> Array1.unsafe_set t.link_indices idx Int32.minus_one
  | Some url ->
      let link_idx = intern_link t url in
      Array1.unsafe_set t.link_indices idx (Int32.of_int link_idx)
...
let ensure_cell_capacity t required =
  if required > t.capacity then (
    ...
    t.chars <- resize_int32_ba t.chars new_capacity;
    ...
    t.link_indices <-
      resize_int32_ba ~default:Int32.minus_one t.link_indices new_capacity;
...
let create_internal ~capacity ~width_method pool =
  let capacity = max 1 capacity in
  {
    ...
    capacity;
    length = 0;
    chars = Array1.create int32 c_layout capacity;
    ...
    link_indices =
      (let arr = Array1.create int32 c_layout capacity in
       for i = 0 to capacity - 1 do
         Array1.unsafe_set arr i Int32.minus_one
       done;
       arr);
...
let reset t =
  ...
  let link_len = Array1.dim t.link_indices in
  for i = 0 to link_len - 1 do
    Array1.unsafe_set t.link_indices i Int32.minus_one
  done
...
let encode_chunk t chunk =
  ...
  let encode_segment segment =
    Glyph.encode t.pool ~width_method:t.width_method segment (fun code ->
...
let newline_code = Int32.of_int 10
...
let compute_lines t =
  if t.lines_dirty then (
    ...
    for idx = 0 to t.length - 1 do
      ...
      let is_newline = Int32.equal code newline_code in
      if is_newline then (
...
let line_body_end t line =
  ...
  if
    line.length > 0
    && Int32.equal (Array1.unsafe_get t.chars (raw_end - 1)) newline_code
  then raw_end - 1
  else raw_end
...
let compute_wrap_break_flags t : (int, int8_unsigned_elt, c_layout) Array1.t =
  ...
  | _ ->
      ...
      for i = 0 to t.length - 1 do
        ...
        if Int32.equal code newline_code then
          ...
          if Glyph.is_continuation code then ()
            ...
            let segment =
              if is_grapheme_id code then Glyph.to_string t.pool code
                ...
                let cp = Int32.to_int code in
                if cp >= 0 && cp < 128 then String.make 1 (Char.chr cp)
            ...
            in
            let breaks = Glyph.wrap_breaks segment in
            if Array.length breaks > 0 then Array1.unsafe_set flags i 1
      done;
...
module View = struct
  type snapshot = {
    chars : (int32, int32_elt, c_layout) Bigarray.Array1.t;
...
let append_code t buf code =
  ...
  else if Int32.to_int code = 10 then Buffer.add_char buf '\n'
  else if is_grapheme_id code then
    Buffer.add_string buf (Glyph.to_string t.pool code)
  else
    let cp = Int32.to_int code in
    if Uchar.is_valid cp then Buffer.add_utf_8_uchar buf (Uchar.of_int cp)
