...
val drawing_chars :
  t -> (int, Bigarray.int_elt, Bigarray.c_layout) Bigarray.Array1.t
...
module View : sig
  ...
  val raw_link : snapshot -> int -> int
  (** [raw_link view idx] returns the raw hyperlink index at index [idx].

      Returns [-1] if no link is present. Use {!link_at_index} to resolve
      indices to URLs.
