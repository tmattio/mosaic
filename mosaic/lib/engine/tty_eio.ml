...
type t = { matrix_tty : Tty.t; input : source; output : sink }
...
let enable_alternate_screen t = write_string t "\0277\027[?1049h"
let disable_alternate_screen t = write_string t "\027[?1049l\0278"
let clear_screen t = write_string t "\027[2J\027[H"
let set_title t title = write_string t (Printf.sprintf "\027]0;%s\007" title)
let bell t = write_string t "\007"

(** {1 Cursor Control} *)

let show_cursor t = write_string t "\027[?25h"
let hide_cursor t = write_string t "\027[?25l"
...
let enable_mouse_sgr t = write_string t "\027[?1006h"
let disable_mouse_sgr t = write_string t "\027[?1006l"

(** {1 Advanced Features} *)

let enable_focus_reporting t = write_string t "\027[?1004h"
let disable_focus_reporting t = write_string t "\027[?1004l"
let enable_bracketed_paste t = write_string t "\027[?2004h"
let disable_bracketed_paste t = write_string t "\027[?2004l"
let enable_kitty_keyboard t = write_string t "\027[>1u"
let disable_kitty_keyboard t = write_string t "\027[<u"

(** {1 Window Management} *)

let set_resize_handler t handler = Tty.set_resize_handler t.matrix_tty handler
let remove_resize_handlers t = Tty.remove_resize_handlers t.matrix_tty

(** {1 Color and Feature Detection} *)

let set_dark_background t ~dark = Tty.set_dark_background t.matrix_tty ~dark
let has_dark_background t = Tty.has_dark_background t.matrix_tty
let has_truecolor_support t = Tty.has_truecolor_support t.matrix_tty
let supports_feature t feature = Tty.supports_feature t.matrix_tty feature
...
let create_from_strings ~sw input =
  ...
  let t =
    { matrix_tty; input = Source input_flow; output = Sink output_flow }
  in
