...
let line ?width ?height ?x_range ?y_range ?(show_axes = true)
    ?(axis_style = Style.dim) ?label_style ?series_styles
    ?(show_grid = false) ?(grid_style = Style.dim)
  ...
  let chart_width = match width with
    | Some (`Cells w) -> Some (`Cells (max 1 (w - y_label_width)))
    | _ -> width
  in
  
  let chart_height = match height with
    | Some (`Cells h) -> Some (`Cells (max 1 (h - x_label_height)))
    | _ -> height
  in
  
  (* Create the main chart canvas *)
  let chart = Canvas.create ?width:chart_width ?height:chart_height (fun ~width:w ~height:h canvas ->
          ...
          | Braille -> (
              let rec plot_braille (prev : point) = function
                ...
                | (p : point) :: tl ->
                    ...
                    let px =
                      int_of_float
                        (scale_value prev.x x_min x_max 0. (float ((plot_w - 1) * 2)))
                      + (ox * 2)
                    in
                    let py =
                      int_of_float
                        (scale_value prev.y y_max y_min 0. (float ((plot_h - 1) * 4)))
                    in
                    let x =
                      int_of_float
                        (scale_value p.x x_min x_max 0. (float ((plot_w - 1) * 2)))
                      + (ox * 2)
                    in
                    let y =
                      int_of_float
                        (scale_value p.y y_max y_min 0. (float ((plot_h - 1) * 4)))
                    in
                    Canvas.draw_line ~x1:px ~y1:py ~x2:x ~y2:y ~style
                      ~kind:`Braille canvas;
                    plot_braille p tl
  ...
  if needs_y_labels || needs_x_labels then (
    ...
    let y_labels = 
      if needs_y_labels then
        ...
        let label_col = Ui.vbox 
          ~justify_content:`Space_between
          ~width:(`Cells y_label_width) [
          Ui.text ~style:label_style (Printf.sprintf "%*s" y_label_width max_label);
          Ui.text ~style:label_style (Printf.sprintf "%*s" y_label_width min_label);
        ] in
        Some label_col
      else None
    in
    ...
    let chart_with_y_labels = match y_labels with
    ...
    | Some labels -> 
        ...
        let x_label_row = 
          if needs_y_labels then
            Ui.hbox ~gap:(`Cells 0) [
              Ui.text (String.make y_label_width ' ');
              labels
            ]
          else labels
        in
...
let bar ?width ?height ?(orientation = `Vertical) ?max_value ?min_value ?bar_width ?gap
  ...
  Canvas.create ?width ?height (fun ~width:w ~height:h canvas ->
      ...
      let all_bar_segments = List.flatten (List.map (fun b -> b.segments) data) in
      let all_segment_values = List.map (fun (s : bar_segment) -> s.value) all_bar_segments in
      let has_negative = List.exists (fun v -> v < 0.0) all_segment_values in
      
      let min_v = Option.value min_value
        ~default:(if has_negative then 
                   List.fold_left min 0. all_segment_values -. 0.0001
                 else 0.0)
      in
      let max_v = Option.value max_value
      ...
      let zero_pos = 
        if has_negative then
          if orientation = `Vertical then
            ...
            int_of_float (scale_value 0.0 min_v max_v 0. (float (w - 1)))
        else
          if orientation = `Vertical then h - 1 else 0
      in
      ...
      if show_axes then (
        ...
        if orientation = `Vertical then (
          ...
          if has_negative then (
            ...
            (* Draw zero line *)
            Canvas.draw_line ~x1:0 ~y1:(h - 1 - zero_pos) ~x2:(w - 1) ~y2:(h - 1 - zero_pos)
              ...
              (fun (seg : bar_segment) ->
                ...
                let seg_size_exact =
                  if orientation = `Vertical then
                    ...
                    abs_value /. (max_v -. min_v) *. float (w - 1)
                in
                ...
                let x, y =
                  if orientation = `Vertical then
                    if is_negative then
                      ...
                      (!pos, h - 1 - zero_pos - seg_size)
                  else
                    if is_negative then
                      (zero_pos - seg_size, !pos)
                    else
                      (zero_pos, !pos)
                in
                ...
                if fraction > 0.125 && seg_size > 0 then (
                  let vertical_blocks = [|" "; "▁"; "▂"; "▃"; "▄"; "▅"; "▆"; "▇"; "█"|] in
                  let horizontal_blocks = [|" "; "▏"; "▎"; "▍"; "▌"; "▋"; "▊"; "▉"; "█"|] in
                  let frac_idx = int_of_float (fraction *. 8.0) |> min 7 |> max 0 in
                  
                  if orientation = `Vertical then (
                    if is_negative && y + bh' < h then (
                      (* Draw partial block at bottom of negative bar *)
                      for xi = x to min (x + bw' - 1) (w - 1) do
                        Canvas.plot canvas ~x:xi ~y:(y + bh') ~style:seg.style 
                          vertical_blocks.(8 - frac_idx)  (* Invert for bottom *)
                      done
                    ...
                    if is_negative && x > 0 then (
                      ...
                      for yi = y to min (y + bh' - 1) (h - 1) do
                        Canvas.plot canvas ~x:(x - 1) ~y:yi ~style:seg.style
                          horizontal_blocks.(8 - frac_idx)  (* Invert for left *)
                      done
  ...
  Canvas.create ?width ?height (fun ~width:w ~height:h canvas ->
      ...
      for y = 0 to h - 1 do
        for x = 0 to w - 1 do
          ...
          Canvas.plot canvas ~x ~y ~style:(Style.fg color) shade_chars.(shade_idx)
        done
      done)
...
let line_with_gaps ?width ?height ?x_range ?y_range ?(show_axes = true)
    ?axis_style ?label_style ?series_styles
    ?(show_grid = false) ?(grid_style = Style.dim)
    ?(render_kind = Lines) (data : (string * (float * float option) list) list) =
      ...
      (fun (name, points_with_gaps) ->
        ...
        List.iter
          (fun (x, y_opt) ->
            match y_opt with
            | Some y ->
                current_segment := { x; y } :: !current_segment
            | None ->
  ...
  line ?width ?height ?x_range ?y_range ~show_axes
    ?axis_style ?label_style ?series_styles ~show_grid ~grid_style
    ~render_kind all_series
