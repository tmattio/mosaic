  ...
  create "typing/ascii-burst" (fun () ->
      let rec loop offset acc =
        if offset >= len then acc
          ...
          let user, caps = I.Parser.feed parser bytes offset chunk in
          loop (offset + chunk) (acc + List.length user + List.length caps)
      in
      let total_events = loop 0 0 in
      ignore (Sys.opaque_identity total_events))
...
let keyboard_legacy_hot =
  ...
  create "keyboard/legacy-hot" (fun () ->
      let user, caps = I.Parser.feed parser bytes 0 len in
      ignore (Sys.opaque_identity (List.length user + List.length caps)))
