...
let set_cell_writes_all_planes () =
  ...
  Grid.set_cell_alpha grid ~x:1 ~y:0
    ~code:(Int32.of_int (Char.code 'A'))
...
let set_cell_outside_scissor_ignored () =
  ...
  Grid.set_cell_alpha grid ~x:0 ~y:0
    ~code:(Int32.of_int (Char.code 'X'))
...
let with_scissor_restores_stack () =
  ...
  Grid.set_cell_alpha grid ~x:1 ~y:1
    ~code:(Int32.of_int (Char.code 'Z'))
    ~fg:Ansi.Color.white ~bg:Ansi.Color.black ~attrs:Ansi.Attr.empty ();
...
let blit_region_blends_without_respect_alpha () =
  ...
  let semi_red = Ansi.Color.of_rgba 255 0 0 128 in
  Grid.set_cell_alpha src ~x:0 ~y:0
    ~code:(Int32.of_int (Char.code 'R'))
...
let scissor_push_overrides_parent () =
  ...
  Grid.set_cell_alpha grid ~x:2 ~y:0
    ~code:(Int32.of_int (Char.code 'B'))
    ~fg:Ansi.Color.white ~bg:Ansi.Color.black ~attrs:Ansi.Attr.empty ();
  Grid.pop_scissor grid;
  Grid.set_cell_alpha grid ~x:0 ~y:0
...
let ambiguous_width_defaults_to_one () =
  let check_width label s =
    let width = Glyph.measure ~width_method:`Unicode s in
...
let canvas_like_resizing () =
  ...
  let write_text text ~x ~y =
    let width = Glyph.measure ~width_method:(Grid.width_method grid) text in
...
let blit_region_copies_subrect () =
  ...
  for y = 0 to 2 do
    for x = 0 to 2 do
      ...
      Grid.set_cell_alpha src ~x ~y ~code:(Int32.of_int char)
        ~fg:Ansi.Color.white ~bg:Ansi.Color.black ~attrs:Ansi.Attr.empty ()
    done
  done;
  ...
  [
    ...
    test_case "box partially off left edge uses correct corners" `Quick
      (fun () ->
        ...
        check int "horizontal at (0,0)"
          (Int32.to_int border_chars.horizontal)
          (read_char grid 0 0);
        check int "top-right corner at (2,0)"
          (Int32.to_int border_chars.top_right)
          (read_char grid 2 0);
        check int "horizontal at (0,2)"
          (Int32.to_int border_chars.horizontal)
          (read_char grid 0 2);
        check int "bottom-right corner at (2,2)"
          (Int32.to_int border_chars.bottom_right)
          (read_char grid 2 2));
    test_case "box partially off top edge extends verticals down" `Quick
      (fun () ->
        ...
        check int "left border at (0,0)"
          (Int32.to_int border_chars.vertical)
          (read_char grid 0 0);
        check int "left border at (0,1)"
          (Int32.to_int border_chars.vertical)
          (read_char grid 0 1);
        check int "left border at (0,2)"
          (Int32.to_int border_chars.vertical)
          (read_char grid 0 2));
    test_case "box partially off right edge uses correct right corners" `Quick
      (fun () ->
        ...
        check int "horizontal at (2,0)"
          (Int32.to_int border_chars.horizontal)
          (read_char grid 2 0);
        check int "horizontal at (2,2)"
          (Int32.to_int border_chars.horizontal)
          (read_char grid 2 2));
    test_case "box fully inside grid works normally" `Quick (fun () ->
        ...
        Grid.draw_box grid ~x:1 ~y:1 ~width:3 ~height:3 ~border_chars
          ...
          ~border_style:Ansi.Style.default ~bg_color:Ansi.Color.black
          ~should_fill:false ();
        check int "top-left corner"
          (Int32.to_int border_chars.top_left)
          (read_char grid 1 1);
        check int "top-right corner"
          (Int32.to_int border_chars.top_right)
          (read_char grid 3 1);
        check int "bottom-left corner"
          (Int32.to_int border_chars.bottom_left)
          (read_char grid 1 3);
        check int "bottom-right corner"
          (Int32.to_int border_chars.bottom_right)
          (read_char grid 3 3));
    (* Diff tests *)
