    ...
    let rec loop lineno =
      try
        ...
        if rule <> "" then (
          ...
          let actual = ref [] in
          iter_graphemes
            (fun off len -> actual := String.sub text off len :: !actual)
            text;
...
let ascii_fast_path_consistency () =
  ...
  check bool "ascii printable accepted" true (is_ascii_printable "abcXYZ");
  check bool "ascii control rejected" false (is_ascii_printable "foo\nbar")

let no_zwj_segmentation () =
  let pool = create_pool () in
  let text = "👩\u{200D}🚀" in
  let cells = encode_to_list pool ~width_method:`No_zwj text in
...
let simple_vs_complex_optimization () =
  ...
  let check_type str expect_simple =
    let lst = encode_to_list pool str in
    match lst with
    | [ g ] ->
  ...
  let del_cells = encode_to_list pool "\x7F" in
...
let complex_clustering_behavior () =
  ...
  let text = "नमस्ते" in
  let cells = encode_to_list pool text in
...
let malformed_utf8_resilience () =
  ...
  let invalid = "\xC3" in
  try
    let cells = encode_to_list pool invalid in
...
let borrowed_storage_roundtrip () =
  let pool = create_pool () in
  let text = "borrow🚀" in
  let g = intern ~borrow:true pool text in
  check string "borrowed to_string" text (to_string pool g);
  let buf = Bytes.create 32 in
  let len = blit pool g buf 0 in
  check int "borrowed blit len" (String.length text) len;
  check string "borrowed blit contents" text (Bytes.sub_string buf 0 len);
  let dst = create_pool () in
  let g_copy = copy pool g dst in
  clear pool;
  check string "copied survives clear" text (to_string dst g_copy)
...
let lifecycle_generation_safety () =
  ...
  (* 4. Allocate NEW glyph. Should reuse slot of g1 but bump generation. *)
  let g2 = intern pool "🛸" in
  let id2 = id g2 in

  check int "slot reused" id1 id2;
...
let pool_resize_stress () =
  ...
  let handles = Array.make n 0l in
...
let copy_safety () =
  ...
  check int32 "simple copy identity" g g_copy;
...
let decref_deduplicates_free_list () =
  ...
  check int "first reuse same slot" (id g) (id g2);
  check int "next allocation uses new slot" 2 (id g3)
...
let tab_expansion_mechanics () =
  ...
  let cells_2 = encode_to_list pool ~tab_width:2 "a\tb" in
  let w_2 =
    List.fold_left (fun acc g -> acc + width ~tab_width:2 g) 0 cells_2
  in
  check int "width with tab=2" (1 + 2 + 1) w_2;

  let cells_4 = encode_to_list pool ~tab_width:4 "a\tb" in
  let w_4 =
...
let tab_cache_offsets () =
  let pool = create_pool () in
  let cells = encode_to_list pool ~tab_width:4 "a\tb\tc" in
  match cells with
  | [ _a; tab1; _b; tab2; _c ] ->
...
let continuation_width_encoding () =
  let pool = create_pool () in
  let cells = encode_to_list pool ~width_method:`Unicode "界" in
...
let line_breaks_lf () =
  let breaks = ref [] in
  iter_line_breaks (fun br -> breaks := br :: !breaks) "a\nb\nc";
...
let line_breaks_cr () =
  let breaks = ref [] in
  iter_line_breaks (fun br -> breaks := br :: !breaks) "a\rb\rc";
  let breaks = List.rev !breaks in
...
let line_breaks_empty () =
  let breaks = ref [] in
  iter_line_breaks (fun br -> breaks := br :: !breaks) "";
  check int "no breaks in empty" 0 (List.length !breaks)

let line_breaks_no_newlines () =
  let breaks = ref [] in
  iter_line_breaks (fun br -> breaks := br :: !breaks) "hello world";
  ...
  [
    ...
    ( "Encoding & Storage",
      [
        ...
        test_case "borrowed roundtrip" `Quick borrowed_storage_roundtrip;
      ] );
    ( "Lifecycle & Pool",
      [
