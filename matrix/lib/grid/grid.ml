...
module Rect = struct
  ...
  (* Helper for row-major iteration (optimization for blit/fill) *)
  let iter_rows r f =
    for y = r.y to r.y + r.height - 1 do
      f ~y ~x_start:r.x ~len:r.width
    done
end
...
module Cell_code = struct
  (* 
    Bit Layout:
    Bit 31: Grapheme Flag (0 = scalar/simple, 1 = complex grapheme/continuation)
    Bit 30: Continuation Flag (0 = start, 1 = continuation)
    Bits 28-29: Right Extent (width - 1)
    Bits 26-27: Left Extent (for continuation cells)
    Bits 0-25:  Payload (Unicode Scalar or Glyph Pool ID)
  *)

  let mask_type = 0xC0000000l
  let flag_grapheme = 0x80000000l
  let flag_continuation = 0xC0000000l (* Both bits set *)
  let mask_payload = 0x03FFFFFFl
  let mask_right_ext = 0x30000000l
  let shift_right_ext = 28
  let mask_left_ext = 0x0C000000l
  let shift_left_ext = 26
  let empty = 0l (* Null/Empty cell *)
  let space = 32l (* ASCII Space *)

  (* Predicates *)
  let[@inline] is_simple c = Int32.logand c flag_grapheme = 0l
  let[@inline] is_complex c = Int32.logand c flag_grapheme = flag_grapheme
  let[@inline] is_continuation c = Int32.logand c mask_type = flag_continuation
  let[@inline] is_start c = is_complex c && not (is_continuation c)

  (* Extraction *)
  let[@inline] payload c = Int32.logand c mask_payload
    ...
    if is_simple c then 1
      ...
      let w_minus_1 =
        Int32.shift_right_logical
          (Int32.logand c mask_right_ext)
          shift_right_ext
      in
      Int32.to_int w_minus_1 + 1

  (* Extent extraction for O(1) cleanup *)
  let[@inline] left_extent c =
    Int32.shift_right_logical (Int32.logand c mask_left_ext) shift_left_ext
    |> Int32.to_int

  let[@inline] right_extent c =
    Int32.shift_right_logical (Int32.logand c mask_right_ext) shift_right_ext
    |> Int32.to_int

  (* Construction *)
  let make_start ~id ~width =
    let w_enc = Int32.of_int (min 3 (max 0 (width - 1))) in
    let bits = Int32.logor flag_grapheme (Int32.logand id mask_payload) in
    Int32.logor bits (Int32.shift_left w_enc shift_right_ext)

  let make_cont ~id ~left ~right =
    let l_enc = Int32.of_int (min 3 left) in
    let r_enc = Int32.of_int (min 3 right) in
    let bits = Int32.logor flag_continuation (Int32.logand id mask_payload) in
    let bits = Int32.logor bits (Int32.shift_left l_enc shift_left_ext) in
    Int32.logor bits (Int32.shift_left r_enc shift_right_ext)
end
...
type t = {
  ...
  (* Storage *)
  mutable chars : (int32, Bigarray.int32_elt) Buf.t;
...
let get_style t idx =
  ...
  let to_color plane idx =
    ...
    let to_byte v = int_of_float (v *. 255.) in
...
let get_text t idx =
  ...
  if Cell_code.is_simple c then (
    match Int32.to_int c with
    | 0 -> ""
    | u ->
        if u < 128 then String.make 1 (Char.chr u)
        else
          let b = Buffer.create 4 in
          Buffer.add_utf_8_uchar b (Uchar.of_int u);
          Buffer.contents b)
  else if Cell_code.is_continuation c then ""
  else
    let id = Cell_code.payload c in
    let complex_code = Int32.logor Cell_code.flag_grapheme id in
    Glyph.to_string t.glyph_pool complex_code
...
let resize t ~width ~height =
  ...
  if width = t.width && height = t.height then ()
    ...
    let new_chars = Buf.make_int32 new_size in
...
let set_cell_internal t ~idx ~code ~fg_r ~fg_g ~fg_b ~fg_a ~bg_r ~bg_g ~bg_b
  ...
  if blending && (bg_a < 0.999 || fg_a < 0.999) then (
    ...
    (if preserve then (
       ...
       let da_fg = Color_plane.get t.fg idx 3 in

       let pa = Color_plane.perceptual_alpha bg_a in
       let r_new = Color_plane.mix bg_r dr_fg pa in
       let g_new = Color_plane.mix bg_g dg_fg pa in
       let b_new = Color_plane.mix bg_b db_fg pa in

       Color_plane.set t.fg idx 0 r_new;
       Color_plane.set t.fg idx 1 g_new;
       Color_plane.set t.fg idx 2 b_new;
       Color_plane.set t.fg idx 3 da_fg
       (* keep dest alpha *))
     else
    ...
    if bg_a <= 0.001 then
      (* No-op: preserve dest BG, as before *)
      ()
    else
      let pa = Color_plane.perceptual_alpha bg_a in
...
let fill_rect t ~x ~y ~width ~height ~color =
  ...
  | Some r ->
      ...
      if bg_a <= 0.001 then
        ...
        Rect.iter_rows r (fun ~y ~x_start ~len ->
            let base_idx = (y * t.width) + x_start in
            for dx = 0 to len - 1 do
              ...
              set_cell_internal t ~idx ~code:space_cell ~fg_r ~fg_g ~fg_b ~fg_a
                ~bg_r:cur_bg_r ~bg_g:cur_bg_g ~bg_b:cur_bg_b ~bg_a:cur_bg_a
        ...
        iter_rows r (fun ~y ~x_start ~len ->
            let start_idx = (y * t.width) + x_start in
            ...
            (* 2. Bulk Set Primitives using Bigarray.fill *)
            Buf.fill (Buf.sub t.chars start_idx len) code;
            Buf.fill (Buf.sub t.attrs start_idx len) attrs;
            Buf.fill (Buf.sub t.links start_idx len) link;
...
let blit ~src ~dst =
  ...
  if src.glyph_pool == dst.glyph_pool then (
    ...
    for i = 0 to len - 1 do
      ...
      if Cell_code.is_complex c then Grapheme_tracker.add dst.grapheme_tracker c
    done)
  else (
    Grapheme_tracker.clear dst.grapheme_tracker;
    ...
    for i = 0 to len - 1 do
      ...
      let dst_c =
        if Cell_code.is_simple src_c then src_c
          ...
          let src_id = Cell_code.payload src_c in
          let dst_id =
            match Hashtbl.find_opt cache src_id with
            | Some id -> id
            | None ->
                let complex_src = Int32.logor Cell_code.flag_grapheme src_id in
                let new_id_raw =
                  Glyph.copy src.glyph_pool complex_src dst.glyph_pool
                in
                let new_id = Cell_code.payload new_id_raw in
                Hashtbl.add cache src_id new_id;
                new_id
          in
          if Cell_code.is_start src_c then
            Cell_code.make_start ~id:dst_id ~width:(Cell_code.width src_c)
          else
            Cell_code.make_cont ~id:dst_id
...
let blit_region ~src ~dst ~src_x ~src_y ~width ~height ~dst_x ~dst_y =
  ...
  if width > 0 && height > 0 then
    ...
    | Some clipped ->
        ...
        if same_grid then
          (* Fast Path: Self-Blit (Scrolling) *)
          let i = ref y_start in
          while !i <> y_limit do
            let sy = src_y + !i in
            let dy = dst_y + !i in
            let src_idx = (sy * src.width) + src_x in
            let dst_idx = (dy * dst.width) + dst_x in
          ...
          let map_payload id =
            match Hashtbl.find_opt grapheme_map id with
            | Some mapped -> mapped
            | None ->
                let complex_src = Int32.logor Cell_code.flag_grapheme id in
                let new_id_raw =
                  Glyph.copy src.glyph_pool complex_src dst.glyph_pool
                in
                let new_id = Cell_code.payload new_id_raw in
                Hashtbl.add grapheme_map id new_id;
                new_id
          in
            ...
            while !k <> x_limit do
              ...
              let final_code_mapped =
                ...
                then code
                  ...
                  let id = map_payload (Cell_code.payload code) in
                  if Cell_code.is_start code then
                    Cell_code.make_start ~id ~width:(Cell_code.width code)
                  else
                    Cell_code.make_cont ~id
              ...
              let blending =
                dst.respect_alpha || fg_a < 0.999 || bg_a < 0.999
              in
...
let draw_text ?style ?(tab_width = 2) t ~x ~y ~text =
  if text = "" || y < 0 || y >= t.height then ()
    ...
    if not row_visible then ()
      ...
      let writer code =
        if Cell_code.is_simple code && Cell_code.payload code = 9l then
          let spaces = tabw in
          ...
          if w > 0 then (
            ...
            (if bounds_ok && start_visible then (
             ...
             else if !cur_x >= 0 && !cur_x < t.width then
      ...
      Glyph.encode t.glyph_pool ~width_method:t.width_method ~tab_width:tabw
        text (fun g ->
          if Glyph.is_continuation g then ()
            ...
            let code =
              if Glyph.is_simple g then g
              else
                let w = Glyph.width g in
                Cell_code.make_start ~id:g ~width:w
            in
            writer code)
  ...
  if width <= 0 || height <= 0 then ()
    ...
    if not scissor_ok then ()
      ...
      let draw_b x y code =
        ...
        then
          let cell =
            if Cell_code.is_simple code then code
            else
              let id = Glyph.intern_char t.glyph_pool (Int32.to_int code) in
              Cell_code.make_start ~id ~width:1
          in
          set_cell_internal t
            ~idx:((y * t.width) + x)
      ...
      | Some txt when has `Top && width >= 4 ->
          let w = Glyph.measure ~width_method:t.width_method txt in
          if w <= width - 4 then
            let pad =
  ...
  if width <= 0 || height <= 0 then ""
    ...
    Ansi.Escape.to_string (fun w ->
        ...
        let close_link () =
          ...
          | None -> ()
          | Some _ ->
              Ansi.Escape.emit Ansi.Escape.hyperlink_end w;
              active_link := None
        in
        ...
        for y = 0 to height - 1 do
          ...
          while !x < width do
            ...
            if cell_w > 0 then (
              (* Hyperlink handling – identical to Screen *)
              let link_opt = hyperlink_url t (get_link t idx) in
              (match (!active_link, link_opt) with
              | Some a, Some b when String.equal a b -> ()
              | _ ->
                  close_link ();
                  Option.iter
                    (fun url ->
                      Ansi.Escape.emit (Ansi.Escape.hyperlink_start ~url) w;
              ...
              (* Emit grapheme *)
              if
                Cell_code.is_continuation code
                || Int32.equal code Cell_code.empty
              then Ansi.Escape.emit (Ansi.Escape.char ' ') w
              else if Cell_code.is_simple code then
                let u = Int32.to_int code in
                if u = 0 then Ansi.Escape.emit (Ansi.Escape.char ' ') w
                else if u < 128 then
                  Ansi.Escape.emit (Ansi.Escape.char (Char.chr u)) w
                else Ansi.Escape.emit (Ansi.Escape.utf8 u) w
              else
                (* Complex grapheme *)
                let payload = Cell_code.payload code in
                let glyph = Int32.logor Cell_code.flag_grapheme payload in
                let len = Glyph.length pool glyph in
                if len <= 0 then Ansi.Escape.emit (Ansi.Escape.char ' ') w
                  ...
                  let written = Glyph.blit pool glyph !scratch 0 in
                  Ansi.Escape.emit
