(** Mutable grid of terminal cells with rich text support.
    ...
    - Printable ASCII (32-126) stored directly as 32-bit codes
    - Multi-width graphemes and non-ASCII text are stored via the glyph pool
    - Wide characters (e.g., CJK) span multiple cells with continuation markers

    This hybrid approach minimizes memory usage while supporting full Unicode.
    ...
    existing cell colors using alpha compositing. This enables semi-transparent
    overlays and smooth color transitions. Use {!set_cell_alpha} to write cells
    with alpha blending, or disable blending globally with {!set_respect_alpha}.
    ...
    Direct access to raw storage ({!raw_chars}, {!raw_fg}, etc.) enables
    zero-copy rendering for performance-critical code paths like terminal output
    diffing.
...
val get_code : t -> int -> int32
(** [get_code t idx] returns the raw cell code at linear index [idx]. This is a
...
(** [blit_region ~src ~dst ~src_x ~src_y ~width ~height ~dst_x ~dst_y] copies a
    ...
    Colors are blended with the destination when [src.respect_alpha] is true and
    the source colors have alpha < 1.0; otherwise cells are overwritten.
...
(** [draw_box t ~x ~y ~width ~height ~border_chars ~border_sides ~border_style
    ...
    @param border_style Style for border characters.
    @param bg_color Background color used when filling.
    @param should_fill
