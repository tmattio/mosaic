(** Text styling composition and management.
    ...
    and hyperlinks into immutable [Style.t] objects. It is optimized for
    efficient composition and zero-allocation ANSI sequence generation.
...
(** [to_sgr_codes ?prev t] calculates the list of SGR integer codes required to
    transition from [prev] to [t].

    - If [prev] is omitted, it defaults to {!default}.
    - The sequence always begins with [0] (Reset) if [t] differs from [prev],
      ensuring a deterministic terminal state before applying new attributes.
...
(** [emit ?prev t writer] writes the necessary SGR codes to [writer].

    This is a zero-allocation operation (excluding internal buffer growth). It
    calculates the minimal state difference between [prev] (defaulting to
    {!default}) and [t] to optimize the output size. *)
...
val emit_raw :
  ...
  Escape.writer ->
  unit
(** [emit_raw ... writer] writes SGR codes directly from raw primitive values.

    {b Purpose}

    This function enables high-performance render loops (such as immediate mode
    GUIs or game engines) to bypass [Style.t] allocation entirely. Callers can
    unpack style data into scalars on the stack and invoke this function.

    {b Behavior}

    It compares the [current] values (fg_*, bg_*, attrs) against the [prev]
    values. If any visual component has changed, it emits a reset code followed
    by the codes for the new state.

    {b Preconditions}

    - Color components (r, g, b, a) must be floats in the range [0.0, 1.0].
    - [attrs] must be a valid integer bitmask compatible with {!Attr.pack}.
    - [prev_*] values must accurately reflect the current state of the writer's
      output to ensure correct delta encoding. *)
