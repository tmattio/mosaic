...
(** [utf8 codepoint] encodes a Unicode scalar value into UTF-8 and writes it.
    ...
    Handles 1-4 byte sequences. Clamps negative values or surrogates to the
    replacement character (U+FFFD) if necessary, or simply ignores invalid
    ranges depending on implementation rigor (usually safe to just encode valid
    Unicode). *)
...
(** [explicit_width_bytes ...] is a zero-copy version of [explicit_width] for
    byte buffers.
