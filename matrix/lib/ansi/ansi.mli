...
(** Streaming ANSI escape sequence parser.
    ...
    Tokenizes ANSI output into high-level tokens. See {!Parser} for details. *)

module Escape = Escape
(** Low-level escape sequence builders.

    Composable, buffer-based emitters for efficient sequence construction. See
    {!Escape} for details. *)

module Sgr_state = Sgr_state
(** Terminal SGR state tracker.

    Tracks the terminal's current style state to minimize emitted SGR codes. See
    {!Sgr_state} for details. *)
...
(** [styled ?reset ?fg ?bg ... str] applies styles to [str].
    ...
    transitions. The [link] parameter emits OSC 8 hyperlinks identical to
    {!Style.hyperlink}.
...
(** [render segments] renders styled segments to a single string.
...
(** [emit ?prev buf segments] streams segments directly into a buffer.

    @param prev Previous style state, defaults to {!Style.default}.
    @param hyperlinks_enabled
      When [false], suppresses OSC 8 sequences. Defaults to [true]. Useful for
      terminals without hyperlink support.
    @param buf Output buffer for rendered segments.
    @param segments List of [(style, text)] pairs to render.

    @return
      Tuple [(final_style, active_link)] where [final_style] is the last style
      applied and [active_link] is [Some url] if a hyperlink is open, [None] if
      closed. The returned state can be fed into subsequent calls to continue
      streaming without redundant resets. *)
...
(** [cursor_save] saves cursor position and character attributes (DEC).

    Must be paired with {!cursor_restore}. *)

val cursor_restore : string
(** [cursor_restore] restores cursor position and attributes saved by
    {!cursor_save}.
