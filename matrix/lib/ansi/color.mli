(** Color representations for ANSI terminal output.
    ...
    indices 0-239, where indices 216-239 represent grayscale levels.
    ...
    - [{!Default}] represents "use terminal default" and is encoded as RGBA
      ...
      that sentinel.
    - Palette indices are coerced into their supported range \[0,239\]
      (extended) or \[0,255\] (full palette) to match terminal behaviour.
    - All conversion helpers preserve semantic equality: converting to packed
      form and back through {!pack}/{!unpack} or {!to_rgba}/{!of_rgba}
      round-trips exactly, including alpha and [{!Default}]'s sentinel value. *)

type rgba = { r : float; g : float; b : float; a : float }
(** Normalized RGBA color channels.

    All components are in the range \[0.0, 1.0\] representing intensity from
    minimum to maximum. Used primarily for internal color space conversions and
    blending operations. *)
...
type t =
  ...
  | Bright_white
  | Extended of int
      (** Extended palette color with index 0-239.

          Indices map to the 256-color palette:
          - 0-215: 6x6x6 RGB cube (r*36 + g*6 + b where r,g,b ∈ \[0,5\])
          - 216-239: 24-level grayscale ramp

          Values outside \[0, 239\] are clamped automatically. Indices 0-215 map
          to 6x6x6 RGB cube, 216-239 to 24-level grayscale. Values ≥240 are
          clamped to 239. Maps to palette indices 16-255 (Extended 0 → palette
          16). *)
...
(** [grayscale ~level] creates a grayscale color from the 24-level ramp.
    ...
    Maps to Extended palette indices 216-239. *)
...
(** [equal a b] tests equality based on RGBA components.
    ...
    For example, [Rgb \{r=255; g=0; b=0\}] equals [Bright_red]. *)
...
(** [to_rgba_f color] converts to normalized RGBA floats in \[0.0, 1.0\].
    ...
    For {!Default}, returns [(0.0, 0.0, 0.0, 0.0)]; the zero alpha plays the
    same sentinel role as in {!to_rgba}. *)

val to_rgba : t -> int * int * int * int
(** [to_rgba color] converts to RGBA integers in \[0, 255\].

    Non-RGBA colors return alpha = 255. {!Default} returns [(0, 0, 0, 0)] with
    alpha = 0 as a sentinel to indicate terminal default. When alpha = 0,
    renderers must detect and substitute configured defaults; compare alpha
    before using RGB components. *)

val to_rgb : t -> int * int * int
(** [to_rgb color] converts to RGB integers in \[0, 255\], discarding alpha.

    [{!Default}] is mapped to [(0, 0, 0)] here; use {!alpha} or {!to_rgba} if
    you need to distinguish it from an explicit black. *)
...
(** [to_sgr_codes ~bg color] converts to SGR parameter codes.
    ...
    - [Red]: \[38; 2; 128; 0; 0\] (fg) or \[48; 2; 128; 0; 0\] (bg)
    - [Bright_red]: \[38; 2; 255; 0; 0\] (fg) or \[48; 2; 255; 0; 0\] (bg)
    - [Extended 42]: \[38; 2; 95; 95; 0\] (fg) or \[48; 2; 95; 95; 0\] (bg)
    - [Rgb \{r=100; g=150; b=200\}]: \[38; 2; 100; 150; 200\] (fg)
    - [Rgba \{r=0; g=0; b=0; a=0\}] when [bg=true]: \[49\] (transparent uses
      terminal default background)

    All explicit colors are emitted as truecolor SGR sequences (38;2 / 48;2).
    For limited terminals, call {!downgrade} before emission. *)

val emit_sgr_codes : bg:bool -> Buffer.t -> t -> unit
(** [emit_sgr_codes ~bg buf color] writes SGR codes directly to buffer.

    Zero-allocation alternative to [to_sgr_codes]. Writes the SGR parameter
    codes for the color directly to the buffer without building intermediate
    lists.

    @param bg If [true], emits background codes; if [false], foreground codes.
    @param buf Buffer to write codes to *)

val emit_sgr_codes_push : bg:bool -> (int -> unit) -> t -> unit
(** [emit_sgr_codes_push ~bg push color] emits SGR codes via a push callback.

    Zero-allocation alternative optimized for direct SGR emission. Calls the
    [push] function for each SGR parameter code without building intermediate
    lists.
