          ...
          if seg_len > 0 then
            try
              ...
              if segment <> "" then (
                ...
                let w = Glyph.measure ~width_method segment in
    ...
    Glyph.iter_graphemes
      (fun off l ->
        ...
        if row < t.rows && t.cursor.col < line_width then
          ...
          let w = Glyph.measure ~width_method cluster in
    ...
    while String.length !remaining > 0 do
      ...
      if t.cursor.row >= t.rows then remaining := ""
        ...
        if available <= 0 then remaining := ""
          ...
          let total_w = Glyph.measure ~width_method s in
...
let feed t bytes ofs len =
  let tokens = Ansi.Parser.feed t.parser bytes ofs len in
