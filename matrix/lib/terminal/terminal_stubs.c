...
CAMLprim value terminal_supports_conpty(value unit_val) {
  CAMLparam1(unit_val);
#ifdef _WIN32
  // ConPTY requires Windows 10 build 17134 (1803) or later
  ...
  if (ntdll) {
    ...
    if (RtlGetVersion) {
      ...
      if (RtlGetVersion(&ver) == 0) {
        // Windows 10 1803 = version 10.0.17134
        if (ver.dwMajorVersion > 10 ||
            (ver.dwMajorVersion == 10 && ver.dwBuildNumber >= 17134)) {
          CAMLreturn(Val_true);
        }
      }
    }
