...
let read_bytes t bytes off len =
  let rec loop () =
    ...
    | exception Unix_error ((EAGAIN | EWOULDBLOCK), _, _) ->
        ignore_unix_errors
          (fun fd -> ignore (Unix.select [ fd ] [] [] (-1.0)))
          t.input;
        loop ()
...
let wait_readable t ~timeout =
  ...
  let rec loop () =
    try
      ...
      let woke = List.exists (fun fd -> fd = t.wakeup_r) read in
      let input_ready = List.exists (fun fd -> fd = t.input) read in
      if woke then (
        ignore_exn
          (fun fd -> ignore (Unix.read fd (Bytes.create 64) 0 64))
          t.wakeup_r;
        if input_ready then true else false)
      else input_ready
    with
...
let take_from_pending t =
  try Some (Queue.take t.pending_events) with Queue.Empty -> None

let apply_capability_events t caps_events =
  if caps_events = [] then ()
  else
    let caps, info =
      Caps.process_events ~apply_env_overrides:t.env_overrides ~caps:t.caps
        ~info:t.terminal_info caps_events
    in
    t.caps <- caps;
...
let next_event ?timeout t =
  let user_deadline =
    match timeout with
    | None -> None
    | Some span -> Some (Unix.gettimeofday () +. max 0. span)
  in
  let handle_user_events = function
    | [] -> None
    | evt :: rest ->
        List.iter (fun e -> Queue.add e t.pending_events) rest;
        Some evt
  and drain_pending () = take_from_pending t in
  let rec loop () =
    match drain_pending () with
    | Some evt -> Some evt
    | None -> (
        let now = Unix.gettimeofday () in
        let flushed, caps_events = Input.Parser.flush ~now t.parser in
        apply_capability_events t caps_events;
        match handle_user_events flushed with
        | Some _ as r -> r
        | None -> (
            let flush_deadline = Input.Parser.next_flush_deadline t.parser in
            let user_wait =
              match user_deadline with
              | Some deadline -> max 0. (deadline -. now)
              | None -> -1.
            in
            let flush_wait =
            ...
            let timeout_span =
              ...
              | false, false ->
                  if user_wait <= flush_wait then user_wait else flush_wait
            in
            let ready = wait_readable t ~timeout:timeout_span in
            if not ready then
              match (timeout, user_deadline) with
              | Some _, Some deadline when Unix.gettimeofday () >= deadline ->
                  None
              | _ -> loop ()
            else
              let rec read_all user_acc caps_acc =
                let read =
                  read_bytes t t.input_buffer 0 (Bytes.length t.input_buffer)
                in
                if read <= 0 then (user_acc, caps_acc)
                else
                  let user_events, caps_events =
                    Input.Parser.feed t.parser t.input_buffer 0 read
                  in
                  let user_acc = user_events @ user_acc in
                  let caps_acc = caps_events @ caps_acc in
                  if wait_readable t ~timeout:0. then read_all user_acc caps_acc
                  else (user_acc, caps_acc)
              in
              let user_events, caps_events = read_all [] [] in
              apply_capability_events t caps_events;
              match handle_user_events (List.rev user_events) with
              | Some _ as r -> r
              | None -> loop ()))
  in
  loop ()
...
let poll_events t =
  ...
  let rec drain_pending acc =
    match take_from_pending t with
    | Some evt -> drain_pending (evt :: acc)
    | None -> List.rev acc
  in
  let events = ref (drain_pending []) in

  (* 2. Flush parser timeouts (e.g., partial sequences that expired) *)
  let flush_parser () =
    let now = Unix.gettimeofday () in
    let user_events, caps_events = Input.Parser.flush ~now t.parser in
    apply_capability_events t caps_events;
    if not (List.is_empty user_events) then events := !events @ user_events
  in

  flush_parser ();

  (* 3. Read all currently available bytes and parse them *)
  let rec read_ready () =
    if wait_readable t ~timeout:0. then
      let read = read_bytes t t.input_buffer 0 (Bytes.length t.input_buffer) in
      if read > 0 then (
        let user_events, caps_events =
          Input.Parser.feed t.parser t.input_buffer 0 read
        in
        apply_capability_events t caps_events;
        if not (List.is_empty user_events) then events := !events @ user_events;
        (* Keep draining while input is ready *)
        read_ready ())
  in
  read_ready ();

  (* 4. Final flush for any sequences that became ready after feed *)
  flush_parser ();
    ...
    with_mode t `Raw (fun () ->
        ...
        let caps, info =
          Caps.probe ~timeout ~apply_env_overrides:t.env_overrides
            ~read_into:(fun buf off len -> read_bytes t buf off len)
...
let register_winch_handler, deregister_winch_handler =
  ...
  let callback _ = List.iter (fun f -> f ()) !handlers in
  let () = Sys.set_signal sigwinch (Sys.Signal_handle callback) in
...
let close t =
  ...
  (match t.winch_handler with
  | Some cb ->
      ignore_exn deregister_winch_handler cb;
      ...
      if now >= deadline then None
        ...
        if not ready then None
          ...
          if read <= 0 then None
            ...
            in
            apply_capability_events t caps_events;
            List.iter (fun e -> Queue.add e t.pending_events) user_events;
            match
              List.find_map
                (function
                  | Input.Caps.Cursor_position (row, col) -> Some (row, col)
                  | _ -> None)
                caps_events
            with
            | Some _ as r -> r
