...
(** Mouse tracking protocols.
    ...
    - [`X10]: basic press-only tracking (legacy; coordinates limited to 223×223
      cells).
    - [`Normal]: press and release events with standard xterm encoding.
    - [`Button]: press, release, and drag events.
    - [`Any]: all motion events, including motion with no buttons pressed.
    - [`Sgr_normal], [`Sgr_button], [`Sgr_any]: SGR-extended variants that
      support terminals larger than 223×223 cells.

    SGR modes automatically enable the SGR extension (1006) and all legacy xterm
    modes (1000 press/release, 1002 drag, 1003 motion) to keep multiplexers such
    as tmux in sync. *)
...
(** Terminal capabilities.
    ...
    The record is monotonic for boolean fields: fields only flip from [false] to
    [true] as more information is discovered. The [unicode_width] field may
    change bidirectionally based on later information. *)
...
(** [open_terminal ?probe ?probe_timeout ?input ?output ?initial_caps ()]
    ...
    - enables VT100 emulation on Windows outputs via [enable_vt]. Failures are
      silently ignored as this is a compatibility enhancement.
...
val poll_events : t -> Input.t list
(** [poll_events t] drains all currently available events without blocking.

    - Flushes any parser timeouts.
    - Reads all ready bytes (non-blocking), feeds them to the parser, and
      collects user events in arrival order.
    - Applies capability responses immediately and excludes them from the
      returned list. *)

val next_event : ?timeout:float -> t -> Input.t option
(** [next_event ?timeout t] returns the next parsed input event.

    - Blocks until an event arrives, the timeout expires, or input ends.
    - Reads bytes from [input], feeds them to the parser, and returns the first
      {!Input.t} produced.
    - Folds any capability responses into {!capabilities} and {!terminal_info}
      and hides them from the returned stream.

    @param timeout
      maximum wait duration in seconds:
      - [timeout < 0.] blocks indefinitely
      - [timeout = 0.] behaves like a non-blocking poll
      - [timeout > 0.] waits up to that many seconds

    Returns [None] on timeout or end-of-input. *)
