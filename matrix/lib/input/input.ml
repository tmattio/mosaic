        ...
        | Some (ux, len2) -> (
            ...
            | Some (uy, len3) ->
                ...
                if x < 0 || y < 0 then None
                  ...
                  let button_code = cb land 3 lor ((cb lsr 6) lsl 6) in
                  let button =
...
let parse_urxvt_mouse s start end_ =
  ...
  | [ Some btn; Some x; Some y ] ->
      ...
      let button_code = base_btn land 3 lor ((base_btn lsr 6) lsl 6) in
      let button =
        match button_code with
        | 0 -> Left
        | 1 -> Middle
          ...
          let event =
            if code = 52 then
              ...
              let decode_base64 s =
                ...
                let out = Buffer.create (len * 3 / 4) in
                let rec decode i =
                  if i + 3 < len then (
