(** Terminal input event handling with keyboard, mouse, and capability tracking.
    ...
    {[
      ...
      let events, caps = Input.Parser.feed parser buffer 0 bytes_read in
    ...
    ({!Key.modifier}), and optional metadata from the Kitty protocol (associated
    ...
    Ambiguous sequences (like a lone Escape key vs. Alt+key) require timing:
    call {!Parser.flush} after a short timeout (typically 10-50ms) to emit
    pending events.
    ...
    Keymaps use first-match semantics. Unspecified modifiers match any state, so
    add more specific bindings before general ones.
