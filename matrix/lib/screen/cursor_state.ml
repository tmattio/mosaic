...
let create () =
  {
    ...
    color = None;
    visible = true;
    (* Initialize "Last" as default terminal assumptions or unknown *)
    last_visible = true;
    last_style = None;
    last_blink = None;
    last_color = None;
...
let hide_temporarily t w =
  if t.last_visible then (
    Esc.emit Esc.hide_cursor w;
    t.last_visible <- false)
...
let emit t ~row_offset w =
  ...
  if not t.visible then (
    ...
    (if t.has_pos then
       ...
       Esc.emit (Esc.cursor_position ~row:r ~col:c) w);
    ...
    (* 5. Ensure Visible (if it was hidden by hide_temporarily or init) *)
    if not t.last_visible then (
