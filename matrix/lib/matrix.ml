...
(* Signal handling to ensure graceful shutdown on SIGTERM/SIGINT *)
...
let () =
  Sys.set_signal Sys.sigterm (Sys.Signal_handle shutdown_signal_handler);
  Sys.set_signal Sys.sigint (Sys.Signal_handle shutdown_signal_handler);
  Sys.set_signal Sys.sigquit (Sys.Signal_handle shutdown_signal_handler);
  Sys.set_signal Sys.sigabrt (Sys.Signal_handle shutdown_signal_handler);
  Printexc.set_uncaught_exception_handler (fun exn ->
      prerr_endline (Printexc.to_string exn);
      run_shutdown_handlers ();
      exit 1);
  at_exit run_shutdown_handlers
...
let create ?(mode = `Alt) ?(raw_mode = true) ?(target_fps = Some 30.)
    ...
    ?(resize_debounce = Some 0.1) ?initial_caps ?(output = `Stdout) () : app =
...
let poll ?timeout t :
    [ `Input of Input.t | `Resize of int * int | `Tick ] option =
  if not t.running then None
    ...
    let has_captured_output =
      ...
      | Some cap -> (
          try
            let ready, _, _ = Unix.select [ cap.read_fd ] [] [] 0. in
            not (List.is_empty ready)
          with _ -> false)
    in
    let immediate_redraw = t.redraw_requested && t.frame_interval = None in
    if has_captured_output then (
      ignore (flush_stdout_capture t);
      t.next_frame_deadline <- Option.map (fun iv -> now +. iv) t.frame_interval;
      Some `Tick)
    else if immediate_redraw || deadline_due then (
      t.next_frame_deadline <- Option.map (fun iv -> now +. iv) t.frame_interval;
      Some `Tick)
    else
      let pending_timeout =
        ...
        | Some _, Some window_s ->
            ...
            let remaining = t.last_resize_apply_time +. window_s -. now2 in
      ...
      in
      let deadline_or_input =
        min_opt (min_opt deadline_timeout t.input_timeout) pending_timeout
      in
      let event_timeout =
        match timeout with
        | None -> deadline_or_input
        | Some tmo when tmo < 0. -> deadline_or_input
        | Some tmo -> min_opt deadline_or_input (Some tmo)
      in
      match Terminal.next_event ?timeout:event_timeout t.terminal with
      | None -> None
      | Some evt -> classify_event t evt)

let run ?on_frame ?on_input ?on_resize ~on_render t =
  if not t.running then t.running <- true;
  (* Ensure the main loop is active when run is invoked, matching prior behavior
     where creation started in Explicit_started. *)
  ...
  (* Drain inputs to prevent death-spiral lag *)
  let rec drain_inputs () =
    if not t.running then ()
    else
      match Terminal.next_event ~timeout:0. t.terminal with
      | None -> ()
      | Some evt -> (
  ...
  let rec loop last_time =
    if not (running t) then ()
      ...
      | Some (`Resize (cols, rows)) ->
          ...
          request_redraw t;
          let next_time =
            if t.redraw_requested then render_cycle ~last_time else last_time
          in
          drain_inputs ();
          loop next_time
      | Some `Tick ->
          let next_time = render_cycle ~last_time in
          drain_inputs ();
          loop next_time
  in
