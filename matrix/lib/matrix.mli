(** Immediate-mode runtime for terminal user interfaces.
    ...
    updates layout). 2. Poll for events via {!poll}. 3. Render into {!grid} and
...
(** [create ()] starts an immediate-mode runtime and returns its handle.
    ...
    @param render_thread
      Whether to use a dedicated thread for output. Defaults to [false]. Enable
      to prevent blocking the main loop on slow terminals, but adds threading
      overhead. Defaults to [true] except on Linux (where it is [false]).
    @param initial_caps
...
val poll :
  ?timeout:float ->
  app ->
  [ `Input of Input.t | `Tick | `Resize of int * int ] option
(** [poll ?timeout app] waits for input or cadence deadlines.

    Returns [`Resize (cols, rows)] when the terminal reports a size change,
    [`Tick] when a cadence fires, [`Input _] for parsed events, or [None] on
    timeout. Resize events auto-dirty the frame. *)
