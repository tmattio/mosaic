...
let%expect_test "table with title and caption" =
  print_ui ~width:30 ~height:10
    (Ui.Table.table
      ...
      ~title_style:Style.(fg Blue ++ bold)
      ~caption_style:Style.(fg (Index 8))
      ());
  [%expect_exact {|
+------------------------------+
|  Sales Report                |
    ...
    (Ui.Table.table
      ~columns:[
        (let c = Ui.Table.default_column ~header:"Name" in
         { c with header_style = Style.(fg Yellow ++ bold) });
        (let c = Ui.Table.default_column ~header:"Value" in
         { c with header_style = Style.(fg Green ++ bold) });
      ]
      ~rows:[["Item1"; "100"]]
      ());
  [%expect_exact {|
    ...
    (Ui.Table.table
      ~columns:[
        (let c = Ui.Table.default_column ~header:"Product" in
         { c with style = Style.(fg Green) });
        (let c = Ui.Table.default_column ~header:"Price" in
         { c with style = Style.(fg Yellow ++ bold) });
      ]
      ~rows:[
        ["Widget"; "$19.99"];
        ["Gadget"; "$49.99"];
      ...
      ~columns:[
        ...
        (let c = Ui.Table.default_column ~header:"Price" in
         { c with 
           ...
           header_style = Style.(fg Green ++ bold)
         });
      ]
      ~rows:[
        ["Widget A"; "$19.99"];
    ...
    (Ui.Table.table
      ...
      ~columns:[
        (let c = Ui.Table.default_column ~header:"Name" in
         { c with 
           min_width = Some 10; 
           header_style = Style.(fg Blue ++ bold);
           footer = Some "Total" 
         });
        (let c = Ui.Table.default_column ~header:"Value" in
         { c with 
      ...
      ~padding:(1, 2, 1, 2)
      ~border_style:Style.(fg Blue)
      ~title_style:Style.(fg Bright_white ++ bold)
      ~footer_style:Style.(fg Yellow ++ bold)
      ());
  [%expect_exact {|
+--------------------------------------------------+
|    Complex Table                                 |
