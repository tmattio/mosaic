...
module InputTestApp = struct
  ...
  let create_app ?placeholder ?initial_value ?password ?suggestions ?validate ?width () =
    let init () =
      let input_model, input_cmd = 
        Input.init ?placeholder ?initial_value ?is_password:password ?suggestions ?validate ?width ()
      in
...
let%expect_test "Password input" =
  let app = InputTestApp.create_app ~placeholder:"Enter password..." ~password:true () in
  let harness = TestHarness.create app in
...
let%expect_test "Input with validation error" =
  let validate input = 
    if String.length input < 5 then 
      ...
      Ok ()
  in
  let app = InputTestApp.create_app ~placeholder:"Min 5 chars..." ~validate () in
  let harness = TestHarness.create app in
...
let%expect_test "Fixed width input" =
  let app = InputTestApp.create_app ~width:20 ~placeholder:"Fixed width..." () in
  let harness = TestHarness.create app in
  
  (* Type text longer than width *)
  TestHarness.push_input "This is a very long text that exceeds the width" harness;
