  ...
  let create_app ?style ?color () =
    ...
    let update msg model =
      ...
      | SpinnerMsg spinner_msg ->
          let new_spinner, spinner_cmd = Spinner.update spinner_msg model.spinner in
...
let%expect_test "Arrow spinner with color" =
  let app = SpinnerTestApp.create_app ~style:Spinner.arrow ~color:(Style.fg Style.Green) () in
  let harness = TestHarness.create app in
  let output = TestHarness.view ~width:20 ~height:3 harness in
  print_test_output output;
  [%expect_exact
...
let%expect_test "Stopped spinner" =
  let app = 
    let init () =
      ...
      ({ SpinnerTestApp.spinner = stopped_spinner; frame_count = 0; quit = false }, 
       Cmd.map (fun m -> SpinnerTestApp.SpinnerMsg m) spinner_cmd)
    in
    let update msg model =
      match msg with
      | SpinnerTestApp.SpinnerMsg spinner_msg ->
          let new_spinner, spinner_cmd = Spinner.update spinner_msg model.SpinnerTestApp.spinner in
    ...
    let view model = 
      let open Ui in
      vbox [
        text (if Spinner.is_spinning model.SpinnerTestApp.spinner then "Spinning" else "Stopped");
        Spinner.view model.SpinnerTestApp.spinner;
      ]
    in
    let subscriptions model =
      Sub.batch [
        Sub.map (fun m -> SpinnerTestApp.SpinnerMsg m) (Spinner.subscriptions model.SpinnerTestApp.spinner);
