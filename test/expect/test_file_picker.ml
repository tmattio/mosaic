...
module FilePickerTestApp = struct
  ...
  let create_app ?start_path ?show_hidden ?directories_only ?extensions ?height () =
    let init () =
      ...
      ( { file_picker = focused_model; quit = false },
        Cmd.batch
          [
            Cmd.map (fun m -> FilePickerMsg m) file_picker_cmd;
            Cmd.map (fun m -> FilePickerMsg m) focus_cmd;
          ] )
    in
    let update msg model =
      match msg with
      | FilePickerMsg file_picker_msg ->
...
let%expect_test "File picker with current directory" =
  let app = FilePickerTestApp.create_app ~start_path:"." ~height:5 () in
  let harness = TestHarness.create app in
...
let%expect_test "Toggle hidden files" =
  ...
  TestHarness.push_key_event (key (Char (Uchar.of_char 'h'))) harness;
