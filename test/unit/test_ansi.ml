...
let test_sgr () =
  ...
  check string "multiple attrs" "\x1b[31;44;1m" (sgr [`Fg Red; `Bg Blue; `Bold]);
  check string "rgb color" "\x1b[38;2;255;128;0m" (sgr [`Fg (RGB (255, 128, 0))]);
...
let test_cursor_style () =
  ...
  check string "blinking underline" "\x1b[3 q" set_cursor_style_blinking_underline;
...
let test_window_title () =
  ...
  check string "title with spaces" "\x1b[]0;My Window\x07" (set_window_title "My Window")
...
let test_strip () =
  ...
  check string "mixed content" "Hello World" (strip "\x1b[31mHello\x1b[0m World");
  
  (* OSC sequences *)
  check string "window title BEL" "Text" (strip "\x1b]0;Title\x07Text");
  check string "window title ST" "Text" (strip "\x1b]0;Title\x1b\\Text");
  check string "hyperlink" "Link" (strip "\x1b]8;;http://example.com\x1b\\Link\x1b]8;;\x1b\\");
  
  (* Other escape sequences *)
  check string "SS2" "" (strip "\x1bNX");  (* SS2 consumes the next char *)
  check string "SS3" "" (strip "\x1bOY");  (* SS3 consumes the next char *)
  ...
  check string "incomplete OSC no terminator" "Start" (strip "Start\x1b]0;Title");
...
let test_hyperlink () =
  check string "simple link" "\x1b]8;;http://example.com\x1b\\Click\x1b]8;;\x1b\\"
  ...
  check string "complex uri" "\x1b]8;;https://example.com/path?q=1&r=2\x1b\\Link\x1b]8;;\x1b\\"
...
let test_color_types () =
  ...
  check string "rgb white" "\x1b[38;2;255;255;255m" (sgr [`Fg (RGB (255, 255, 255))]);
  check string "rgb custom" "\x1b[38;2;128;64;192m" (sgr [`Fg (RGB (128, 64, 192))])
...
let test_style_attrs () =
  ...
  List.iter (fun (attr, expected) ->
    check string (Printf.sprintf "%s" (Obj.magic attr)) expected (sgr [attr])
  ) styles
...
let () =
  run "ANSI"
    [
      ...
      ("Cursor movement", [ test_case "cursor movement" `Quick test_cursor_movement ]);
