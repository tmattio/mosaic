...
let test_set_char_regular () =
  ...
  (* Unicode character *)
  set_char buf 1 0 (Uchar.of_int 0x03B1) style; (* Greek alpha *)
...
let test_set_char_wide () =
  ...
  set_char buf 9 0 emoji style; (* Should not write continuation cell *)
...
let test_set_char_combining () =
  ...
  set_char buf 3 2 acute style; (* x=3 but should combine with x=2 *)
...
let test_set_string () =
  ...
  for i = 0 to 4 do
    ...
    check uchar_list (Printf.sprintf "char %d" i) 
...
let test_set_string_malformed () =
  ...
  List.iteri (fun i expected ->
    if i < 5 then
      check (testable pp_uchar Uchar.equal) (Printf.sprintf "char %d" i) 
        (Uchar.of_char expected) (List.nth chars i)
  ) ['H'; 'e'; 'l'; 'l'; 'o'];
  
  (* Replacement characters *)
  check (testable pp_uchar Uchar.equal) "replacement 1" 
    (Uchar.of_int 0xFFFD) (List.nth chars 5);
  check (testable pp_uchar Uchar.equal) "replacement 2" 
    (Uchar.of_int 0xFFFD) (List.nth chars 6)
...
let test_render_patch () =
  ...
  check bool "contains position" true 
    (String.contains rendered '6'); (* row+1 *)
  check bool "contains X" true 
    (String.contains rendered 'X')
...
let test_render_patches () =
  ...
  check bool "ends with reset" true 
    (String.ends_with ~suffix:"\x1b[0m" rendered ||
     String.split_on_char '\x1b' rendered |> List.exists (String.starts_with ~prefix:"[0m"));
  ...
  check bool "shows cursor" true 
    (String.split_on_char '\x1b' rendered |> List.exists (String.starts_with ~prefix:"[?25h"))
...
let test_render_full () =
  ...
  check bool "contains clear" true 
    (String.contains rendered 'J');
...
let test_measure_string () =
  ...
  check int "emoji string" 5 (measure_string "Hi😀!"); (* 2+2+1 *)
  check int "mixed string" 12 (measure_string "Test 测试 OK"); (* 5 + 4 + 3 *)
