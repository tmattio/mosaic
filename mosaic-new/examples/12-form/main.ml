      ...
      box ~flex_direction:Column ~gap:(gap 2) ~border:true ~padding:(padding 2)
        ...
        [
          ...
          box ~flex_direction:Row ~align_items:Flex_start ~gap:(gap 1)
            [
              ...
              select ~options:roles ~show_description:true
                ~selected_background:Ansi.Color.blue ~selected_index:model.role
                ~on_change:(fun idx -> Some (Set_role idx))
                ~size:{ width = px 25; height = px 5 }
                ();
            ];
          (* Buttons *)
          box ~flex_direction:Row ~gap:(gap 2) ~margin:(margin 1)
            [
          ...
          (if model.submitted then
           ...
           else text ~content:"Fill in the form and click Submit" ());
          (* Help *)
          text ~content:"Tab to navigate fields, Enter to submit, 'q' to quit"
            ();
        ];
    ]
