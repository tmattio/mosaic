...
(* Converts a range of tracks as indexes into the CellOccupancyMatrix into start
   and end grid lines in OriginZero coordinates *)
...
(* Converts an OriginZero grid line into a track index for the
   CellOccupancyMatrix, or None if the line is outside the grid *)
