...
let test_grid_min_content_flex_row_border_box measure_function () =
  (* Setup test helpers *)
  ...
  let _ =
    set_node_context tree node3 (Some (MeasureFunction.Text "HH​HH"))
    |> Result.get_ok
  in
...
let test_grid_min_content_flex_row_content_box measure_function () =
  (* Setup test helpers *)
  let assert_eq ~msg expected actual =
    let open Alcotest in
    check (float 0.001) msg expected actual
...
let tests =
  let open Alcotest in
  [
    test_case "min_content_flex_row (border-box)" `Quick (fun () ->
        test_grid_min_content_flex_row_border_box measure_function ());
    test_case "min_content_flex_row (content-box)" `Quick (fun () ->
        test_grid_min_content_flex_row_content_box measure_function ());
  ]
