...
let test_measure_child_with_flex_shrink_hidden_border_box () =
  (* Setup test helpers *)
  ...
  (* Create nodes *)
  let node = Toffee.new_leaf tree ({ Toffee.Style.default with size = { width = Toffee.Style.Dimension.length 100.0; height = Toffee.Style.Dimension.auto } }) in
  ...
  let _ = Toffee.set_node_context tree node1 (MeasureFunction.Fixed {width = 540.0; height = 10.0}) |> Result.get_ok in
  ...
  let layout = layout |> Result.get_ok in
  assert_eq ~msg:"width of node1" 91.0 layout.size.width;
  assert_eq ~msg:"height of node1" 50.0 layout.size.height;
  assert_eq ~msg:"x of node1" 9.0 layout.location.x;
  assert_eq ~msg:"y of node1" 0.0 layout.location.y;
  ()



let test_measure_child_with_flex_shrink_hidden_content_box () =
  (* Setup test helpers *)
  let assert_eq ~msg expected actual =
    let open Alcotest in
    check (float 0.001) msg expected actual
  ...
  (* Create nodes *)
  let node = Toffee.new_leaf tree ({ Toffee.Style.default with size = { width = Toffee.Style.Dimension.length 100.0; height = Toffee.Style.Dimension.auto }; box_sizing = Toffee.Style.Content_box }) in
...
let () =
  let open Alcotest in
  run "Toffee measure_child_with_flex_shrink_hidden Test" [
    "flex_measure_child_with_flex_shrink_hidden", [
      test_case "measure_child_with_flex_shrink_hidden (border-box)" `Quick test_measure_child_with_flex_shrink_hidden_border_box;
      test_case "measure_child_with_flex_shrink_hidden (content-box)" `Quick test_measure_child_with_flex_shrink_hidden_content_box;
    ];
  ]
