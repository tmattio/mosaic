...
let test_flex_measure_width_overrides_measure_border_box measure_function () =
  (* Setup test helpers *)
  ...
  in
  let _ =
    set_node_context tree node1 (Some (MeasureFunction.Text "HHHHHHHHHH"))
    |> Result.get_ok
  in
  let node0 =
...
let test_flex_measure_width_overrides_measure_content_box measure_function () =
  (* Setup test helpers *)
  let assert_eq ~msg expected actual =
    let open Alcotest in
    check (float 0.001) msg expected actual
...
let tests =
  let open Alcotest in
  [
    test_case "measure_width_overrides_measure (border-box)" `Quick (fun () ->
        test_flex_measure_width_overrides_measure_border_box measure_function ());
    test_case "measure_width_overrides_measure (content-box)" `Quick (fun () ->
        test_flex_measure_width_overrides_measure_content_box measure_function
          ());
  ]
