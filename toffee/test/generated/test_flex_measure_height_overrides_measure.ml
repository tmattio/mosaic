...
let test_flex_measure_height_overrides_measure_border_box () =
  (* Setup test helpers *)
  ...
  let _ =
    set_node_context tree node1
      (Some
         (Gentest_helpers.ahem_text "H" Gentest_helpers.Writing_mode.Horizontal))
    |> Result.get_ok
  in
  let node0 =
...
let test_flex_measure_height_overrides_measure_content_box () =
  (* Setup test helpers *)
  let assert_eq ~msg expected actual =
    let open Alcotest in
    check (float 0.001) msg expected actual
...
let tests =
  ...
  [
    test_case "measure_height_overrides_measure (border-box)" `Quick
      test_flex_measure_height_overrides_measure_border_box;
    test_case "measure_height_overrides_measure (content-box)" `Quick
      test_flex_measure_height_overrides_measure_content_box;
  ]
