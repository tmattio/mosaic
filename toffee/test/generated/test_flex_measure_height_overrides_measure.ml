...
(* Generated test for measure_height_overrides_measure in flex layout *)
...
let test_measure_height_overrides_measure_border_box () =
  (* Setup test helpers *)
  ...
  let node0 = Toffee.new_leaf tree ({ Toffee.Style.default with size = { width = Toffee.Style.Dimension.auto; height = Toffee.Style.Dimension.length 5.0 } }) in
  let _ = Toffee.set_node_context tree node0 (MeasureFunction.Fixed {width = 10.0; height = 10.0}) |> Result.get_ok in
  ...
  assert_eq ~msg:"height of node" 5.0 layout.size.height;
  ...
  assert_eq ~msg:"height of node0" 5.0 layout.size.height;
...
let test_measure_height_overrides_measure_content_box () =
  (* Setup test helpers *)
  let assert_eq ~msg expected actual =
    let open Alcotest in
    check (float 0.001) msg expected actual
  ...
  let node0 = Toffee.new_leaf tree ({ Toffee.Style.default with size = { width = Toffee.Style.Dimension.auto; height = Toffee.Style.Dimension.length 5.0 }; box_sizing = Toffee.Style.Content_box }) in
  let _ = Toffee.set_node_context tree node0 (MeasureFunction.Fixed {width = 10.0; height = 10.0}) |> Result.get_ok in
  let _ = Toffee.add_child tree node node0 |> Result.get_ok in
...
let () =
  let open Alcotest in
  run "Toffee measure_height_overrides_measure Test" [
    "flex_measure_height_overrides_measure", [
      test_case "measure_height_overrides_measure (border-box)" `Quick test_measure_height_overrides_measure_border_box;
      test_case "measure_height_overrides_measure (content-box)" `Quick test_measure_height_overrides_measure_content_box;
    ];
  ]
