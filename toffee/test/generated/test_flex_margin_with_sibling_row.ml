...
let test_flex_margin_with_sibling_row_border_box () =
  (* Setup test helpers *)
  ...
  assert_eq ~msg:"width of node2" 45.0 (Layout.size layout_result).width;
...
let test_flex_margin_with_sibling_row_content_box () =
  (* Setup test helpers *)
  let assert_eq ~msg expected actual =
    let open Alcotest in
    check (float 0.001) msg expected actual
...
let tests =
  let open Alcotest in
  [
    test_case "margin_with_sibling_row (border-box)" `Quick
      test_flex_margin_with_sibling_row_border_box;
    test_case "margin_with_sibling_row (content-box)" `Quick
      test_flex_margin_with_sibling_row_content_box;
  ]
