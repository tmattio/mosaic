...
let test_flex_overflow_main_axis_shrink_scroll_border_box measure_function () =
  (* Setup test helpers *)
  ...
  assert_eq ~msg:"scroll_width of node1" 50.0
...
let test_flex_overflow_main_axis_shrink_scroll_content_box measure_function () =
  (* Setup test helpers *)
  let assert_eq ~msg expected actual =
    let open Alcotest in
    check (float 0.001) msg expected actual
...
let tests =
  let open Alcotest in
  [
    test_case "overflow_main_axis_shrink_scroll (border-box)" `Quick (fun () ->
        test_flex_overflow_main_axis_shrink_scroll_border_box measure_function
          ());
    test_case "overflow_main_axis_shrink_scroll (content-box)" `Quick (fun () ->
        test_flex_overflow_main_axis_shrink_scroll_content_box measure_function
          ());
  ]
