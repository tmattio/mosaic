...
let test_grid_minmax_column_fixed_width_below_range_border_box () =
  ...
  let node =
    new_leaf tree
      (Style.make ~display:Style.Display.Grid
         ...
         ~size:
           {
             width = Style.Dimension.length 90.0;
  ...
  assert_eq ~msg:"width of node" 90.0 (Layout.size layout_result).width;
...
let tests =
  let open Alcotest in
  [
    test_case "minmax_column_fixed_width_below_range (border-box)" `Quick
      test_grid_minmax_column_fixed_width_below_range_border_box;
    test_case "minmax_column_fixed_width_below_range (content-box)" `Quick
      test_grid_minmax_column_fixed_width_below_range_content_box;
  ]
