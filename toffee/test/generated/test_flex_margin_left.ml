...
let test_flex_margin_left_border_box () =
  (* Setup test helpers *)
...
let test_flex_margin_left_content_box () =
  (* Setup test helpers *)
  let assert_eq ~msg expected actual =
    let open Alcotest in
    check (float 0.001) msg expected actual
...
let tests =
  let open Alcotest in
  [
    test_case "margin_left (border-box)" `Quick test_flex_margin_left_border_box;
    test_case "margin_left (content-box)" `Quick
      test_flex_margin_left_content_box;
  ]
