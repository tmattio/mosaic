...
let test_grid_aspect_ratio_fill_child_max_height_border_box measure_function ()
  ...
  let _ =
    set_node_context tree node1
      (Some (MeasureFunction.Text_vertical "HH​HH​HH​HH​HH​HH"))
    |> Result.get_ok
  in
  let node0 =
...
let test_grid_aspect_ratio_fill_child_max_height_content_box measure_function ()
    =
  (* Setup test helpers *)
  let assert_eq ~msg expected actual =
    let open Alcotest in
...
let tests =
  ...
  [
    test_case "aspect_ratio_fill_child_max_height (border-box)" `Quick
      (fun () ->
        test_grid_aspect_ratio_fill_child_max_height_border_box measure_function
          ());
