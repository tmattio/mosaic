...
(* Generated test for grid_minmax_auto_percent_indefinite in grid layout *)
...
let test_grid_minmax_auto_percent_indefinite_border_box () =
  (* Setup test helpers *)
  ...
  (* Create nodes *)
  let node = Toffee.new_leaf tree ({ Toffee.Style.default with display = Toffee.Style.Grid; grid_template_columns = [Toffee.Style.Grid.Single { min = Toffee.Style.Grid.Auto; max = Toffee.Style.Grid.Percent (0.2) }]; grid_template_rows = [Toffee.Style.Grid.Single { min = Toffee.Style.Grid.Length (40.0); max = Toffee.Style.Grid.Length (40.0) }] }) in
  ...
  let layout = layout |> Result.get_ok in
  assert_eq ~msg:"width of node0" 8.0 layout.size.width;
...
let test_grid_minmax_auto_percent_indefinite_content_box () =
  (* Setup test helpers *)
  let assert_eq ~msg expected actual =
    let open Alcotest in
    check (float 0.001) msg expected actual
...
let () =
  let open Alcotest in
  run "Toffee grid_minmax_auto_percent_indefinite Test" [
    "grid_grid_minmax_auto_percent_indefinite", [
      test_case "grid_minmax_auto_percent_indefinite (border-box)" `Quick test_grid_minmax_auto_percent_indefinite_border_box;
      test_case "grid_minmax_auto_percent_indefinite (content-box)" `Quick test_grid_minmax_auto_percent_indefinite_content_box;
    ];
  ]
