...
let test_block_margin_y_collapse_through_positive_and_negative_border_box () =
         ...
         ~margin:
           {
             ...
             bottom = Style.Length_percentage_auto.length (-4.0);
           }
         ~box_sizing:Style.Box_sizing.Border_box ())
    |> Result.get_ok
  in
  ...
  assert_eq ~msg:"height of node0" 17.0 (Layout.size layout_result).height;
...
let tests =
  let open Alcotest in
  [
    test_case "margin_y_collapse_through_positive_and_negative (border-box)"
      `Quick
      test_block_margin_y_collapse_through_positive_and_negative_border_box;
    test_case "margin_y_collapse_through_positive_and_negative (content-box)"
      `Quick
      test_block_margin_y_collapse_through_positive_and_negative_content_box;
  ]
