...
(* Generated test for grid_repeat_mixed in grid layout *)
...
let test_grid_repeat_mixed_border_box () =
  (* Setup test helpers *)
...
let test_grid_repeat_mixed_content_box () =
  (* Setup test helpers *)
  let assert_eq ~msg expected actual =
    let open Alcotest in
    check (float 0.001) msg expected actual
...
let () =
  let open Alcotest in
  run "Toffee grid_repeat_mixed Test" [
    "grid_grid_repeat_mixed", [
      test_case "grid_repeat_mixed (border-box)" `Quick test_grid_repeat_mixed_border_box;
      test_case "grid_repeat_mixed (content-box)" `Quick test_grid_repeat_mixed_content_box;
    ];
  ]
