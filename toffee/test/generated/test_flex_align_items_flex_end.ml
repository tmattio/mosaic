...
let test_flex_align_items_flex_end_border_box () =
  (* Setup test helpers *)
  ...
  let node =
    new_leaf tree
      (Style.make ~align_items:Flex_end
         ~size:
           {
...
let test_flex_align_items_flex_end_content_box () =
  (* Setup test helpers *)
  let assert_eq ~msg expected actual =
    let open Alcotest in
    check (float 0.001) msg expected actual
...
let tests =
  let open Alcotest in
  [
    test_case "align_items_flex_end (border-box)" `Quick
      test_flex_align_items_flex_end_border_box;
    test_case "align_items_flex_end (content-box)" `Quick
      test_flex_align_items_flex_end_content_box;
  ]
