...
let test_flex_simple_child_border_box () =
  (* Setup test helpers *)
  ...
  in
  let node4 =
    new_with_children tree Style.default [| node5; node6 |] |> Result.get_ok
  in
  let node1 =
    new_with_children tree
      (Style.make
  ...
  assert_eq ~msg:"y of node6" 45.0 (Layout.location layout_result).y;
...
let test_flex_simple_child_content_box () =
  (* Setup test helpers *)
  let assert_eq ~msg expected actual =
    let open Alcotest in
    check (float 0.001) msg expected actual
...
let tests =
  let open Alcotest in
  [
    test_case "simple_child (border-box)" `Quick
      test_flex_simple_child_border_box;
    test_case "simple_child (content-box)" `Quick
      test_flex_simple_child_content_box;
  ]
