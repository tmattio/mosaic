...
let test_grid_placement_definite_primary_border_box () =
  ...
  (* Create nodes *)
  let node = Toffee.new_leaf tree ({ Toffee.Style.default with display = Toffee.Style.Grid; grid_template_columns = [Toffee.Style.Grid.Single { min = Toffee.Style.Grid.Percent (0.33); max = Toffee.Style.Grid.Percent (0.33) }; Toffee.Style.Grid.Single { min = Toffee.Style.Grid.Percent (0.74); max = Toffee.Style.Grid.Percent (0.74) }]; size = { width = Toffee.Style.Dimension.length 200.0; height = Toffee.Style.Dimension.length 200.0 } }) in
  ...
  let layout = layout |> Result.get_ok in
  assert_eq ~msg:"width of node0" 66.0 layout.size.width;
  ...
  assert_eq ~msg:"height of node1" 67.0 layout.size.height;
  assert_eq ~msg:"x of node1" 66.0 layout.location.x;
  assert_eq ~msg:"y of node1" 0.0 layout.location.y;
  let layout = Toffee.layout tree node2 in
  let layout = layout |> Result.get_ok in
  assert_eq ~msg:"width of node2" 66.0 layout.size.width;
  assert_eq ~msg:"height of node2" 66.0 layout.size.height;
  assert_eq ~msg:"x of node2" 0.0 layout.location.x;
  assert_eq ~msg:"y of node2" 67.0 layout.location.y;
  let layout = Toffee.layout tree node3 in
  let layout = layout |> Result.get_ok in
  assert_eq ~msg:"width of node3" 148.0 layout.size.width;
  assert_eq ~msg:"height of node3" 66.0 layout.size.height;
  assert_eq ~msg:"x of node3" 66.0 layout.location.x;
  assert_eq ~msg:"y of node3" 67.0 layout.location.y;
  let layout = Toffee.layout tree node4 in
  let layout = layout |> Result.get_ok in
  assert_eq ~msg:"width of node4" 66.0 layout.size.width;
  assert_eq ~msg:"height of node4" 67.0 layout.size.height;
  assert_eq ~msg:"x of node4" 0.0 layout.location.x;
  assert_eq ~msg:"y of node4" 133.0 layout.location.y;
  ()
...
let () =
  let open Alcotest in
  run "Toffee grid_placement_definite_primary Test" [
    "grid_grid_placement_definite_primary", [
      test_case "grid_placement_definite_primary (border-box)" `Quick test_grid_placement_definite_primary_border_box;
      test_case "grid_placement_definite_primary (content-box)" `Quick test_grid_placement_definite_primary_content_box;
    ];
  ]
