...
(* Generated test for grid_repeat_integer in grid layout *)
...
let test_grid_repeat_integer_border_box () =
  (* Setup test helpers *)
  ...
  (* Create nodes *)
  let node = Toffee.new_leaf tree ({ Toffee.Style.default with display = Toffee.Style.Grid; grid_template_columns = [Toffee.Style.Grid.Single { min = Toffee.Style.Grid.Auto; max = Toffee.Style.Grid.Auto }]; grid_template_rows = [Toffee.Style.Grid.Single { min = Toffee.Style.Grid.Auto; max = Toffee.Style.Grid.Auto }]; size = { width = Toffee.Style.Dimension.length 120.0; height = Toffee.Style.Dimension.length 120.0 } }) in
...
let test_grid_repeat_integer_content_box () =
  (* Setup test helpers *)
  let assert_eq ~msg expected actual =
    let open Alcotest in
    check (float 0.001) msg expected actual
...
let () =
  let open Alcotest in
  run "Toffee grid_repeat_integer Test" [
    "grid_grid_repeat_integer", [
      test_case "grid_repeat_integer (border-box)" `Quick test_grid_repeat_integer_border_box;
      test_case "grid_repeat_integer (content-box)" `Quick test_grid_repeat_integer_content_box;
    ];
  ]
