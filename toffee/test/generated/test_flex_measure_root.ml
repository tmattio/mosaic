...
let test_measure_root_border_box () =
  (* Setup test helpers *)
...
let test_measure_root_content_box () =
  (* Setup test helpers *)
  let assert_eq ~msg expected actual =
    let open Alcotest in
    check (float 0.001) msg expected actual
...
let () =
  let open Alcotest in
  run "Toffee measure_root Test"
    [
      ( "flex_measure_root",
        [
          test_case "measure_root (border-box)" `Quick
            test_measure_root_border_box;
          test_case "measure_root (content-box)" `Quick
            test_measure_root_content_box;
        ] );
    ]
