...
let test_measure_root_border_box () =
  ...
  let _ = Toffee.set_node_context tree node (MeasureFunction.Fixed {width = 60.0; height = 10.0}) |> Result.get_ok in
...
let () =
  ...
  run "Toffee measure_root Test" [
    "flex_measure_root", [
      test_case "measure_root (border-box)" `Quick test_measure_root_border_box;
      test_case "measure_root (content-box)" `Quick test_measure_root_content_box;
    ];
  ]
