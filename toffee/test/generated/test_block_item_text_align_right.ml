  ...
  let node0 =
    new_with_children tree
      (Style.make ~display:Style.Display.Block
         ...
         ~text_align:Style.Text_align.Legacy_right ())
      [| node1; node2 |]
    |> Result.get_ok
  in
...
let tests =
  let open Alcotest in
  [
    test_case "item_text_align_right (border-box)" `Quick
      test_block_item_text_align_right_border_box;
    test_case "item_text_align_right (content-box)" `Quick
      test_block_item_text_align_right_content_box;
  ]
