...
let test_flex_overflow_main_axis_shrink_visible_border_box () =
  (* Setup test helpers *)
  ...
  let node1 =
    new_leaf tree
      (Style.make ~flex_shrink:1.0 ~box_sizing:Style.Box_sizing.Border_box ())
    |> Result.get_ok
  in
  let _ =
...
let test_flex_overflow_main_axis_shrink_visible_content_box () =
  (* Setup test helpers *)
  let assert_eq ~msg expected actual =
    Windtrap.equal ~msg (Windtrap.Testable.float 0.001) expected actual
  in
...
let tests =
  [
    Windtrap.test "overflow_main_axis_shrink_visible (border-box)"
      test_flex_overflow_main_axis_shrink_visible_border_box;
    Windtrap.test "overflow_main_axis_shrink_visible (content-box)"
      test_flex_overflow_main_axis_shrink_visible_content_box;
  ]
