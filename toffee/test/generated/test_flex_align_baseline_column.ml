...
let test_flex_align_baseline_column_border_box () =
  (* Setup test helpers *)
  ...
  let node =
    new_leaf tree
      (Style.make ~display:Style.Display.Flex
         ~flex_direction:Style.Flex_direction.Column ~align_items:Baseline
         ~size:
           {
...
let test_flex_align_baseline_column_content_box () =
  (* Setup test helpers *)
  let assert_eq ~msg expected actual =
    let open Alcotest in
    check (float 0.001) msg expected actual
...
let tests =
  let open Alcotest in
  [
    test_case "align_baseline_column (border-box)" `Quick
      test_flex_align_baseline_column_border_box;
    test_case "align_baseline_column (content-box)" `Quick
      test_flex_align_baseline_column_content_box;
  ]
