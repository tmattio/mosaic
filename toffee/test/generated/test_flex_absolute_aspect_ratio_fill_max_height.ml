...
let test_flex_absolute_aspect_ratio_fill_max_height_border_box measure_function
    () =
  (* Setup test helpers *)
  ...
  assert_eq ~msg:"height of node1" 17.0 (Layout.size layout_result).height;
...
let test_flex_absolute_aspect_ratio_fill_max_height_content_box measure_function
    () =
  (* Setup test helpers *)
  let assert_eq ~msg expected actual =
    let open Alcotest in
  ...
  let node1 =
    new_leaf tree
      (Style.make ~position:Style.Position.Absolute
         ...
         ~aspect_ratio:3.0 ~box_sizing:Style.Box_sizing.Content_box ())
    |> Result.get_ok
  in
  let _ =
    set_node_context tree node1
...
let tests =
  let open Alcotest in
  [
    test_case "absolute_aspect_ratio_fill_max_height (border-box)" `Quick
      (fun () ->
        test_flex_absolute_aspect_ratio_fill_max_height_border_box
          measure_function ());
    test_case "absolute_aspect_ratio_fill_max_height (content-box)" `Quick
      (fun () ->
        test_flex_absolute_aspect_ratio_fill_max_height_content_box
          measure_function ());
  ]
