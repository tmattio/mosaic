...
(* Generated test for flex_grow_child in flex layout *)
...
let test_flex_grow_child_border_box () =
  (* Setup test helpers *)
  ...
  let node0 = Toffee.new_leaf tree ({ Toffee.Style.default with flex_grow = 1.0; flex_basis = Toffee.Style.Dimension.length 0.0; size = { width = Toffee.Style.Dimension.auto; height = Toffee.Style.Dimension.length 100.0 } }) in
...
let test_flex_grow_child_content_box () =
  (* Setup test helpers *)
  let assert_eq ~msg expected actual =
    let open Alcotest in
    check (float 0.001) msg expected actual
  ...
  let node0 = Toffee.new_leaf tree ({ Toffee.Style.default with flex_grow = 1.0; flex_basis = Toffee.Style.Dimension.length 0.0; size = { width = Toffee.Style.Dimension.auto; height = Toffee.Style.Dimension.length 100.0 }; box_sizing = Toffee.Style.Content_box }) in
  let _ = Toffee.add_child tree node node0 |> Result.get_ok in
...
let () =
  let open Alcotest in
  run "Toffee flex_grow_child Test" [
    "flex_flex_grow_child", [
      test_case "flex_grow_child (border-box)" `Quick test_flex_grow_child_border_box;
      test_case "flex_grow_child (content-box)" `Quick test_flex_grow_child_content_box;
    ];
  ]
