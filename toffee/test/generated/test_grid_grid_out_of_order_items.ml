...
(* Generated test for grid_out_of_order_items in grid layout *)
...
let test_grid_out_of_order_items_border_box () =
  (* Setup test helpers *)
  ...
  let node2 = Toffee.new_leaf tree ({ Toffee.Style.default with size = { width = Toffee.Style.Dimension.length 35.0; height = Toffee.Style.Dimension.length 35.0 } }) in
  ...
  assert_eq ~msg:"x of node0" 80.0 layout.location.x;
  ...
  let layout = layout |> Result.get_ok in
  assert_eq ~msg:"width of node2" 35.0 layout.size.width;
  assert_eq ~msg:"height of node2" 35.0 layout.size.height;
...
let test_grid_out_of_order_items_content_box () =
  (* Setup test helpers *)
  let assert_eq ~msg expected actual =
    let open Alcotest in
    check (float 0.001) msg expected actual
  ...
  let _ = Toffee.add_child tree node node1 |> Result.get_ok in
  let node2 = Toffee.new_leaf tree ({ Toffee.Style.default with size = { width = Toffee.Style.Dimension.length 35.0; height = Toffee.Style.Dimension.length 35.0 }; box_sizing = Toffee.Style.Content_box }) in
  let _ = Toffee.add_child tree node node2 |> Result.get_ok in
...
let () =
  let open Alcotest in
  run "Toffee grid_out_of_order_items Test" [
    "grid_grid_out_of_order_items", [
      test_case "grid_out_of_order_items (border-box)" `Quick test_grid_out_of_order_items_border_box;
      test_case "grid_out_of_order_items (content-box)" `Quick test_grid_out_of_order_items_content_box;
    ];
  ]
