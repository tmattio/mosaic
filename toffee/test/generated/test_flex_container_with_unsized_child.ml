...
(* Generated test for container_with_unsized_child in flex layout *)
...
let test_container_with_unsized_child_border_box () =
  (* Setup test helpers *)
...
let test_container_with_unsized_child_content_box () =
  (* Setup test helpers *)
  let assert_eq ~msg expected actual =
    let open Alcotest in
    check (float 0.001) msg expected actual
...
let () =
  let open Alcotest in
  run "Toffee container_with_unsized_child Test" [
    "flex_container_with_unsized_child", [
      test_case "container_with_unsized_child (border-box)" `Quick test_container_with_unsized_child_border_box;
      test_case "container_with_unsized_child (content-box)" `Quick test_container_with_unsized_child_content_box;
    ];
  ]
