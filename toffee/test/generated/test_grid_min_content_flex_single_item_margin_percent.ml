...
let test_grid_min_content_flex_single_item_margin_percent_border_box
  ...
  in
  let _ =
    set_node_context tree node4 (Some (MeasureFunction.Text "HH​HH"))
    |> Result.get_ok
  in
  ...
  assert_eq ~msg:"height of node1" 38.0 (Layout.size layout_result).height;
  assert_eq ~msg:"x of node1" 42.0 (Layout.location layout_result).x;
  ...
  assert_eq ~msg:"width of node4" 28.0 (Layout.size layout_result).width;
  assert_eq ~msg:"height of node4" 32.0 (Layout.size layout_result).height;
  assert_eq ~msg:"x of node4" 48.0 (Layout.location layout_result).x;
  assert_eq ~msg:"y of node4" 42.0 (Layout.location layout_result).y;
...
let tests =
  let open Alcotest in
  [
    test_case "min_content_flex_single_item_margin_percent (border-box)" `Quick
      (fun () ->
        test_grid_min_content_flex_single_item_margin_percent_border_box
          measure_function ());
    test_case "min_content_flex_single_item_margin_percent (content-box)" `Quick
      (fun () ->
