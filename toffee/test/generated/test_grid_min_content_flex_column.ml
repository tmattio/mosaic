...
let test_grid_min_content_flex_column_border_box measure_function () =
  (* Setup test helpers *)
...
let test_grid_min_content_flex_column_content_box measure_function () =
  (* Setup test helpers *)
  let assert_eq ~msg expected actual =
    let open Alcotest in
    check (float 0.001) msg expected actual
...
let tests =
  let open Alcotest in
  [
    test_case "min_content_flex_column (border-box)" `Quick (fun () ->
        test_grid_min_content_flex_column_border_box measure_function ());
    test_case "min_content_flex_column (content-box)" `Quick (fun () ->
        test_grid_min_content_flex_column_content_box measure_function ());
  ]
