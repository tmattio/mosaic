  ...
  let node =
    new_leaf tree
      (Style.make ~display:Style.Display.Grid
         ...
         ~gap:
           {
             width = Style.Length_percentage.length 40.0;
             height = Style.Length_percentage.length 40.0;
           }
         ())
    |> Result.get_ok
  in
...
let tests =
  let open Alcotest in
  [
    test_case "gap (border-box)" `Quick test_grid_gap_border_box;
    test_case "gap (content-box)" `Quick test_grid_gap_content_box;
  ]
