...
let test_grid_relayout_vertical_text_border_box measure_function () =
  (* Setup test helpers *)
...
let test_grid_relayout_vertical_text_content_box measure_function () =
  (* Setup test helpers *)
  let assert_eq ~msg expected actual =
    let open Alcotest in
    check (float 0.001) msg expected actual
...
let tests =
  let open Alcotest in
  [
    test_case "relayout_vertical_text (border-box)" `Quick (fun () ->
        test_grid_relayout_vertical_text_border_box measure_function ());
    test_case "relayout_vertical_text (content-box)" `Quick (fun () ->
        test_grid_relayout_vertical_text_content_box measure_function ());
  ]
