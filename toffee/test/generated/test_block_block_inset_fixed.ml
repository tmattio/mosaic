...
(* Generated test for block_inset_fixed in block layout *)
...
let test_block_inset_fixed_border_box () =
  (* Setup test helpers *)
  ...
  assert_eq ~msg:"x of node2" 2.0 layout.location.x;
  assert_eq ~msg:"y of node2" 24.0 layout.location.y;
  ()



let test_block_inset_fixed_content_box () =
  (* Setup test helpers *)
  let assert_eq ~msg expected actual =
    let open Alcotest in
    check (float 0.001) msg expected actual
...
let () =
  let open Alcotest in
  run "Toffee block_inset_fixed Test" [
    "block_block_inset_fixed", [
      test_case "block_inset_fixed (border-box)" `Quick test_block_inset_fixed_border_box;
      test_case "block_inset_fixed (content-box)" `Quick test_block_inset_fixed_content_box;
    ];
  ]
