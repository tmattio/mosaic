...
let test_grid_margins_percent_end_border_box () =
  ...
  let node1 =
    new_leaf tree
      (Style.make ~align_self:End ~justify_self:Style.Align_items.End
         ~size:
           {
  ...
  assert_eq ~msg:"x of node1" (-2.0) (Layout.location layout_result).x;
...
let tests =
  let open Alcotest in
  [
    test_case "margins_percent_end (border-box)" `Quick
      test_grid_margins_percent_end_border_box;
    test_case "margins_percent_end (content-box)" `Quick
      test_grid_margins_percent_end_content_box;
  ]
