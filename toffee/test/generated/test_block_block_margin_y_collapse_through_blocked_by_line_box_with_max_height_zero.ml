...
(* Generated test for block_margin_y_collapse_through_blocked_by_line_box_with_max_height_zero in block layout *)
...
let test_block_margin_y_collapse_through_blocked_by_line_box_with_max_height_zero_border_box () =
  (* Setup test helpers *)
  ...
  let node1 = Toffee.new_leaf tree ({ Toffee.Style.default with display = Toffee.Style.Block; max_size = { width = Toffee.Style.Dimension.auto; height = Toffee.Style.Dimension.length 0.0 }; margin = { left = Toffee.Style.Length_percentage_auto.Length (0.0); right = Toffee.Style.Length_percentage_auto.Length (0.0); top = Toffee.Style.Length_percentage_auto.Length (10.0); bottom = Toffee.Style.Length_percentage_auto.Length (10.0) } }) in
  let _ = Toffee.set_node_context tree node1 (MeasureFunction.Fixed {width = 20.0; height = 10.0}) |> Result.get_ok in
...
let test_block_margin_y_collapse_through_blocked_by_line_box_with_max_height_zero_content_box () =
  (* Setup test helpers *)
  let assert_eq ~msg expected actual =
    let open Alcotest in
    check (float 0.001) msg expected actual
...
let () =
  let open Alcotest in
  run "Toffee block_margin_y_collapse_through_blocked_by_line_box_with_max_height_zero Test" [
    "block_block_margin_y_collapse_through_blocked_by_line_box_with_max_height_zero", [
      test_case "block_margin_y_collapse_through_blocked_by_line_box_with_max_height_zero (border-box)" `Quick test_block_margin_y_collapse_through_blocked_by_line_box_with_max_height_zero_border_box;
      test_case "block_margin_y_collapse_through_blocked_by_line_box_with_max_height_zero (content-box)" `Quick test_block_margin_y_collapse_through_blocked_by_line_box_with_max_height_zero_content_box;
    ];
  ]
