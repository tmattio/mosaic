...
let test_flex_gap_column_gap_child_margins_border_box () =
  (* Setup test helpers *)
  ...
  let node3 =
    new_leaf tree
      (Style.make ~flex_grow:1.0 ~flex_shrink:1.0
         ...
         ~margin:
           {
             left = Style.Length_percentage_auto.length 15.0;
  ...
  assert_eq ~msg:"x of node2" 26.0 (Layout.location layout_result).x;
...
let test_flex_gap_column_gap_child_margins_content_box () =
  (* Setup test helpers *)
  let assert_eq ~msg expected actual =
    let open Alcotest in
    check (float 0.001) msg expected actual
...
let tests =
  let open Alcotest in
  [
    test_case "gap_column_gap_child_margins (border-box)" `Quick
      test_flex_gap_column_gap_child_margins_border_box;
    test_case "gap_column_gap_child_margins (content-box)" `Quick
      test_flex_gap_column_gap_child_margins_content_box;
  ]
