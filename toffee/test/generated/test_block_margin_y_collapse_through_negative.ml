...
let test_block_margin_y_collapse_through_negative_border_box () =
  ...
  assert_eq ~msg:"y of node3" 3.0 (Layout.location layout_result).y;
  let layout_result = layout tree node0 |> Result.get_ok in
  assert_eq ~msg:"width of node0" 50.0 (Layout.size layout_result).width;
  assert_eq ~msg:"height of node0" 13.0 (Layout.size layout_result).height;
...
let tests =
  let open Alcotest in
  [
    test_case "margin_y_collapse_through_negative (border-box)" `Quick
      test_block_margin_y_collapse_through_negative_border_box;
    test_case "margin_y_collapse_through_negative (content-box)" `Quick
      test_block_margin_y_collapse_through_negative_content_box;
  ]
