...
(* Generated test for grid_auto_takes_precedence_over_fr in grid layout *)
...
let test_grid_auto_takes_precedence_over_fr_border_box () =
  (* Setup test helpers *)
  ...
  (* Create nodes *)
  let node = Toffee.new_leaf tree ({ Toffee.Style.default with display = Toffee.Style.Grid; grid_template_columns = [Toffee.Style.Grid.Single { min = Toffee.Style.Grid.Auto; max = Toffee.Style.Grid.Auto }; Toffee.Style.Grid.Single { min = Toffee.Style.Grid.Auto; max = Toffee.Style.Grid.Fr (1.0) }]; grid_template_rows = [Toffee.Style.Grid.Single { min = Toffee.Style.Grid.Length (40.0); max = Toffee.Style.Grid.Length (40.0) }]; size = { width = Toffee.Style.Dimension.length 200.0; height = Toffee.Style.Dimension.length 200.0 } }) in
...
let test_grid_auto_takes_precedence_over_fr_content_box () =
  (* Setup test helpers *)
  let assert_eq ~msg expected actual =
    let open Alcotest in
    check (float 0.001) msg expected actual
...
let () =
  let open Alcotest in
  run "Toffee grid_auto_takes_precedence_over_fr Test" [
    "grid_grid_auto_takes_precedence_over_fr", [
      test_case "grid_auto_takes_precedence_over_fr (border-box)" `Quick test_grid_auto_takes_precedence_over_fr_border_box;
      test_case "grid_auto_takes_precedence_over_fr (content-box)" `Quick test_grid_auto_takes_precedence_over_fr_content_box;
    ];
  ]
