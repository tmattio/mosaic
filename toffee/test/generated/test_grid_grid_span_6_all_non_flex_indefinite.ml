...
(* Generated test for grid_span_6_all_non_flex_indefinite in grid layout *)
...
let test_grid_span_6_all_non_flex_indefinite_border_box () =
  (* Setup test helpers *)
  ...
  let layout = layout |> Result.get_ok in
  assert_eq ~msg:"width of node2" 89.0 layout.size.width;
  ...
  assert_eq ~msg:"x of node3" 99.0 layout.location.x;
  assert_eq ~msg:"y of node3" 40.0 layout.location.y;
  let layout = Toffee.layout tree node4 in
  let layout = layout |> Result.get_ok in
  assert_eq ~msg:"width of node4" 9.0 layout.size.width;
  assert_eq ~msg:"height of node4" 40.0 layout.size.height;
  assert_eq ~msg:"x of node4" 109.0 layout.location.x;
...
let test_grid_span_6_all_non_flex_indefinite_content_box () =
  (* Setup test helpers *)
  let assert_eq ~msg expected actual =
    let open Alcotest in
    check (float 0.001) msg expected actual
...
let () =
  let open Alcotest in
  run "Toffee grid_span_6_all_non_flex_indefinite Test" [
    "grid_grid_span_6_all_non_flex_indefinite", [
      test_case "grid_span_6_all_non_flex_indefinite (border-box)" `Quick test_grid_span_6_all_non_flex_indefinite_border_box;
      test_case "grid_span_6_all_non_flex_indefinite (content-box)" `Quick test_grid_span_6_all_non_flex_indefinite_content_box;
    ];
  ]
