...
let test_flex_align_flex_start_with_stretching_children_border_box () =
  (* Setup test helpers *)
  ...
  let node1 =
    new_with_children tree
      (Style.make ~align_items:Stretch ~box_sizing:Style.Box_sizing.Border_box
         ())
      [| node2 |]
    |> Result.get_ok
  in
  ...
  assert_eq ~msg:"height of node3" 500.0 (Layout.size layout_result).height;
  ...
  assert_eq ~msg:"height of node2" 500.0 (Layout.size layout_result).height;
...
let test_flex_align_flex_start_with_stretching_children_content_box () =
  (* Setup test helpers *)
  let assert_eq ~msg expected actual =
    let open Alcotest in
    check (float 0.001) msg expected actual
...
let tests =
  let open Alcotest in
  [
    test_case "align_flex_start_with_stretching_children (border-box)" `Quick
      test_flex_align_flex_start_with_stretching_children_border_box;
    test_case "align_flex_start_with_stretching_children (content-box)" `Quick
      test_flex_align_flex_start_with_stretching_children_content_box;
  ]
