...
let test_flex_measure_child_border_box () =
  (* Setup test helpers *)
...
let test_flex_measure_child_content_box () =
  (* Setup test helpers *)
  let assert_eq ~msg expected actual =
    let open Alcotest in
    check (float 0.001) msg expected actual
...
let tests =
  let open Alcotest in
  [
    test_case "measure_child (border-box)" `Quick
      test_flex_measure_child_border_box;
    test_case "measure_child (content-box)" `Quick
      test_flex_measure_child_content_box;
  ]
