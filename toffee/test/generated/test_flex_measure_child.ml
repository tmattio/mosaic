...
let test_measure_child_border_box () =
  (* Setup test helpers *)
...
let test_measure_child_content_box () =
  (* Setup test helpers *)
  let assert_eq ~msg expected actual =
    let open Alcotest in
    check (float 0.001) msg expected actual
...
let () =
  let open Alcotest in
  run "Toffee measure_child Test"
    [
      ( "flex_measure_child",
        [
          test_case "measure_child (border-box)" `Quick
            test_measure_child_border_box;
          test_case "measure_child (content-box)" `Quick
            test_measure_child_content_box;
        ] );
    ]
