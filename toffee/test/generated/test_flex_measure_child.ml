(* Generated test for measure_child in flex layout *)
...
let test_flex_measure_child_border_box measure_function () =
  (* Setup test helpers *)
  ...
  let _ =
    set_node_context tree node1 (Some (MeasureFunction.Text "HHHHHH"))
    |> Result.get_ok
  in
  let node0 =
...
let test_flex_measure_child_content_box measure_function () =
  (* Setup test helpers *)
  let assert_eq ~msg expected actual =
    let open Alcotest in
    check (float 0.001) msg expected actual
...
let tests =
  let open Alcotest in
  [
    test_case "measure_child (border-box)" `Quick (fun () ->
        test_flex_measure_child_border_box measure_function ());
    test_case "measure_child (content-box)" `Quick (fun () ->
        test_flex_measure_child_content_box measure_function ());
  ]
