...
let test_block_absolute_child_with_margin_y_border_box () =
         ...
         ~margin:
           {
             ...
             bottom = Style.Length_percentage_auto.length 7.0;
           }
         ())
    |> Result.get_ok
  in
  ...
  assert_eq ~msg:"y of node1" 7.0 (Layout.location layout_result).y;
...
let tests =
  let open Alcotest in
  [
    test_case "absolute_child_with_margin_y (border-box)" `Quick
      test_block_absolute_child_with_margin_y_border_box;
    test_case "absolute_child_with_margin_y (content-box)" `Quick
      test_block_absolute_child_with_margin_y_content_box;
  ]
