...
let test_flex_grow_0_min_size_border_box measure_function () =
  (* Setup test helpers *)
  ...
  in
  let _ =
    Toffee.set_node_context tree node0 (MeasureFunction.Text "one")
    |> Result.get_ok
  in
  ...
  in
  let _ =
    Toffee.set_node_context tree node1 (MeasureFunction.Text "two")
    |> Result.get_ok
  in
  ...
  in
  let _ =
    Toffee.set_node_context tree node2 (MeasureFunction.Text "three")
    |> Result.get_ok
  in
  ...
  in
  let _ =
    Toffee.set_node_context tree node3 (MeasureFunction.Text "four")
    |> Result.get_ok
  in
...
let tests =
  let open Alcotest in
  [
    test_case "grow_0_min_size (border-box)" `Quick (fun () ->
        test_flex_grow_0_min_size_border_box measure_function ());
    test_case "grow_0_min_size (content-box)" `Quick (fun () ->
        test_flex_grow_0_min_size_content_box measure_function ());
  ]
