...
let test_flex_basis_flex_grow_column_border_box () =
  ...
  let node0 =
    new_leaf tree
      (Style.make ~flex_grow:1.0 ~flex_basis:(Style.Dimension.length 50.0) ())
    |> Result.get_ok
  in
...
let tests =
  let open Alcotest in
  [
    test_case "basis_flex_grow_column (border-box)" `Quick
      test_flex_basis_flex_grow_column_border_box;
    test_case "basis_flex_grow_column (content-box)" `Quick
      test_flex_basis_flex_grow_column_content_box;
  ]
