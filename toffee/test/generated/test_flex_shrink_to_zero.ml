...
let test_flex_shrink_to_zero_border_box () =
  ...
  let node0 =
    new_with_children tree
      (Style.make
         ~size:
           {
             width = Style.Dimension.length 75.0;
  ...
  assert_eq ~msg:"width of node0" 75.0 (Layout.size layout_result).width;
...
let tests =
  let open Alcotest in
  [
    test_case "shrink_to_zero (border-box)" `Quick
      test_flex_shrink_to_zero_border_box;
    test_case "shrink_to_zero (content-box)" `Quick
      test_flex_shrink_to_zero_content_box;
  ]
