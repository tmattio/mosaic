...
let test_flex_wrapped_row_within_align_items_flex_start_border_box () =
  (* Setup test helpers *)
  ...
  let node1 =
    new_with_children tree
      (Style.make ~flex_direction:Style.Flex_direction.Row
         ~flex_wrap:Style.Flex_wrap.Wrap ~box_sizing:Style.Box_sizing.Border_box
         ())
      [| node2; node3 |]
    |> Result.get_ok
  in
  ...
  assert_eq ~msg:"height of node1" 160.0 (Layout.size layout_result).height;
...
let test_flex_wrapped_row_within_align_items_flex_start_content_box () =
  (* Setup test helpers *)
  let assert_eq ~msg expected actual =
    let open Alcotest in
    check (float 0.001) msg expected actual
...
let tests =
  let open Alcotest in
  [
    test_case "wrapped_row_within_align_items_flex_start (border-box)" `Quick
      test_flex_wrapped_row_within_align_items_flex_start_border_box;
    test_case "wrapped_row_within_align_items_flex_start (content-box)" `Quick
      test_flex_wrapped_row_within_align_items_flex_start_content_box;
  ]
