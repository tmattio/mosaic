...
(* Generated test for grid_basic_implicit_tracks in grid layout *)
...
let test_grid_basic_implicit_tracks_border_box () =
  (* Setup test helpers *)
  ...
  let node1 = Toffee.new_leaf tree ({ Toffee.Style.default with size = { width = Toffee.Style.Dimension.length 35.0; height = Toffee.Style.Dimension.length 35.0 } }) in
  ...
  assert_eq ~msg:"height of node" 75.0 layout.size.height;
  ...
  let layout = layout |> Result.get_ok in
  assert_eq ~msg:"width of node1" 35.0 layout.size.width;
  assert_eq ~msg:"height of node1" 35.0 layout.size.height;
...
let test_grid_basic_implicit_tracks_content_box () =
  (* Setup test helpers *)
  let assert_eq ~msg expected actual =
    let open Alcotest in
    check (float 0.001) msg expected actual
  ...
  let _ = Toffee.add_child tree node node0 |> Result.get_ok in
  let node1 = Toffee.new_leaf tree ({ Toffee.Style.default with size = { width = Toffee.Style.Dimension.length 35.0; height = Toffee.Style.Dimension.length 35.0 }; box_sizing = Toffee.Style.Content_box }) in
  let _ = Toffee.add_child tree node node1 |> Result.get_ok in
...
let () =
  let open Alcotest in
  run "Toffee grid_basic_implicit_tracks Test" [
    "grid_grid_basic_implicit_tracks", [
      test_case "grid_basic_implicit_tracks (border-box)" `Quick test_grid_basic_implicit_tracks_border_box;
      test_case "grid_basic_implicit_tracks (content-box)" `Quick test_grid_basic_implicit_tracks_content_box;
    ];
  ]
