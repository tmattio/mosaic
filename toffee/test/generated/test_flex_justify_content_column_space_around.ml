...
let test_flex_justify_content_column_space_around_border_box () =
  (* Setup test helpers *)
  ...
  assert_eq ~msg:"y of node3" 78.0 (Layout.location layout_result).y;
...
let test_flex_justify_content_column_space_around_content_box () =
  (* Setup test helpers *)
  let assert_eq ~msg expected actual =
    let open Alcotest in
    check (float 0.001) msg expected actual
...
let tests =
  let open Alcotest in
  [
    test_case "justify_content_column_space_around (border-box)" `Quick
      test_flex_justify_content_column_space_around_border_box;
    test_case "justify_content_column_space_around (content-box)" `Quick
      test_flex_justify_content_column_space_around_content_box;
  ]
