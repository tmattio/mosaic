...
(* Generated test for grid_relayout_vertical_text in grid layout *)
...
let test_grid_relayout_vertical_text_border_box () =
  (* Setup test helpers *)
  ...
  let _ = Toffee.set_node_context tree node0 (MeasureFunction.Fixed {width = 200.0; height = 10.0}) |> Result.get_ok in
...
let test_grid_relayout_vertical_text_content_box () =
  (* Setup test helpers *)
  let assert_eq ~msg expected actual =
    let open Alcotest in
    check (float 0.001) msg expected actual
...
let () =
  let open Alcotest in
  run "Toffee grid_relayout_vertical_text Test" [
    "grid_grid_relayout_vertical_text", [
      test_case "grid_relayout_vertical_text (border-box)" `Quick test_grid_relayout_vertical_text_border_box;
      test_case "grid_relayout_vertical_text (content-box)" `Quick test_grid_relayout_vertical_text_content_box;
    ];
  ]
