...
let test_grid_minmax_auto_percent_indefinite_border_box measure_function () =
  (* Setup test helpers *)
  ...
  assert_eq ~msg:"width of node0" 8.0 (Layout.size layout_result).width;
...
let test_grid_minmax_auto_percent_indefinite_content_box measure_function () =
  (* Setup test helpers *)
  let assert_eq ~msg expected actual =
    let open Alcotest in
    check (float 0.001) msg expected actual
...
let tests =
  let open Alcotest in
  [
    test_case "minmax_auto_percent_indefinite (border-box)" `Quick (fun () ->
        test_grid_minmax_auto_percent_indefinite_border_box measure_function ());
    test_case "minmax_auto_percent_indefinite (content-box)" `Quick (fun () ->
        test_grid_minmax_auto_percent_indefinite_content_box measure_function ());
  ]
