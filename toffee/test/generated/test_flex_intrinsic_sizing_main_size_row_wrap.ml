...
let test_intrinsic_sizing_main_size_row_wrap_border_box () =
  (* Setup test helpers *)
  ...
  (* Create nodes *)
  let node = Toffee.new_leaf tree ({ Toffee.Style.default with flex_wrap = Toffee.Style.Flex.Wrap }) in
...
let test_intrinsic_sizing_main_size_row_wrap_content_box () =
  (* Setup test helpers *)
  let assert_eq ~msg expected actual =
    let open Alcotest in
    check (float 0.001) msg expected actual
  ...
  (* Create nodes *)
  let node = Toffee.new_leaf tree ({ Toffee.Style.default with flex_wrap = Toffee.Style.Flex.Wrap; box_sizing = Toffee.Style.Content_box }) in
...
let () =
  let open Alcotest in
  run "Toffee intrinsic_sizing_main_size_row_wrap Test" [
    "flex_intrinsic_sizing_main_size_row_wrap", [
      test_case "intrinsic_sizing_main_size_row_wrap (border-box)" `Quick test_intrinsic_sizing_main_size_row_wrap_border_box;
      test_case "intrinsic_sizing_main_size_row_wrap (content-box)" `Quick test_intrinsic_sizing_main_size_row_wrap_content_box;
    ];
  ]
