...
let test_padding_align_end_child_border_box () =
  (* Setup test helpers *)
  ...
  (* Create nodes *)
  let node = Toffee.new_leaf tree ({ Toffee.Style.default with align_items = Some (Toffee.Style.Alignment.Flex_end); justify_content = Some (Toffee.Style.Alignment.Flex_end); size = { width = Toffee.Style.Dimension.length 200.0; height = Toffee.Style.Dimension.length 200.0 } }) in
...
let test_padding_align_end_child_content_box () =
  (* Setup test helpers *)
  let assert_eq ~msg expected actual =
    let open Alcotest in
    check (float 0.001) msg expected actual
  ...
  (* Create nodes *)
  let node = Toffee.new_leaf tree ({ Toffee.Style.default with align_items = Some (Toffee.Style.Alignment.Flex_end); justify_content = Some (Toffee.Style.Alignment.Flex_end); size = { width = Toffee.Style.Dimension.length 200.0; height = Toffee.Style.Dimension.length 200.0 }; box_sizing = Toffee.Style.Content_box }) in
  let node0 = Toffee.new_leaf tree ({ Toffee.Style.default with size = { width = Toffee.Style.Dimension.length 100.0; height = Toffee.Style.Dimension.length 100.0 }; padding = { left = Toffee.Style.Length_percentage.Length (20.0); right = Toffee.Style.Length_percentage.Length (20.0); top = Toffee.Style.Length_percentage.Length (20.0); bottom = Toffee.Style.Length_percentage.Length (20.0) }; box_sizing = Toffee.Style.Content_box }) in
  let _ = Toffee.add_child tree node node0 |> Result.get_ok in
...
let () =
  let open Alcotest in
  run "Toffee padding_align_end_child Test" [
    "flex_padding_align_end_child", [
      test_case "padding_align_end_child (border-box)" `Quick test_padding_align_end_child_border_box;
      test_case "padding_align_end_child (content-box)" `Quick test_padding_align_end_child_content_box;
    ];
  ]
