(* Generated test for rounding_inner_node_controversy_combined in flex layout *)
(* Do not edit this file directly. It is generated by gentest.js *)

open Toffee

let test_flex_rounding_inner_node_controversy_combined_border_box () =
  (* Setup test helpers *)
  let assert_eq ~msg expected actual =
    Windtrap.equal ~msg (Windtrap.Testable.float 0.001) expected actual
  in

  let tree = Gentest_helpers.new_test_tree () in

  (* Create nodes *)
  let node1 =
    new_leaf tree
      (Style.make ~flex_grow:1.0
         ~size:
           {
             width = Style.Dimension.auto;
             height = Style.Dimension.percent 1.0;
           }
         ~box_sizing:Style.Box_sizing.Border_box ())
    |> Result.get_ok
  in
  let node3 =
    new_leaf tree
      (Style.make ~flex_grow:1.0
         ~size:
           {
             width = Style.Dimension.percent 1.0;
             height = Style.Dimension.auto;
           }
         ~box_sizing:Style.Box_sizing.Border_box ())
    |> Result.get_ok
  in
  let node5 =
    new_leaf tree
      (Style.make ~flex_grow:1.0
         ~size:
           {
             width = Style.Dimension.percent 1.0;
             height = Style.Dimension.auto;
           }
         ~box_sizing:Style.Box_sizing.Border_box ())
    |> Result.get_ok
  in
  let node4 =
    new_with_children tree
      (Style.make ~flex_direction:Style.Flex_direction.Column ~flex_grow:1.0
         ~size:
           {
             width = Style.Dimension.percent 1.0;
             height = Style.Dimension.auto;
           }
         ~box_sizing:Style.Box_sizing.Border_box ())
      [| node5 |]
    |> Result.get_ok
  in
  let node6 =
    new_leaf tree
      (Style.make ~flex_grow:1.0
         ~size:
           {
             width = Style.Dimension.percent 1.0;
             height = Style.Dimension.auto;
           }
         ~box_sizing:Style.Box_sizing.Border_box ())
    |> Result.get_ok
  in
  let node2 =
    new_with_children tree
      (Style.make ~flex_direction:Style.Flex_direction.Column ~flex_grow:1.0
         ~size:
           {
             width = Style.Dimension.auto;
             height = Style.Dimension.percent 1.0;
           }
         ~box_sizing:Style.Box_sizing.Border_box ())
      [| node3; node4; node6 |]
    |> Result.get_ok
  in
  let node7 =
    new_leaf tree
      (Style.make ~flex_grow:1.0
         ~size:
           {
             width = Style.Dimension.auto;
             height = Style.Dimension.percent 1.0;
           }
         ~box_sizing:Style.Box_sizing.Border_box ())
    |> Result.get_ok
  in
  let node0 =
    new_with_children tree
      (Style.make ~flex_direction:Style.Flex_direction.Row
         ~size:
           {
             width = Style.Dimension.length 640.0;
             height = Style.Dimension.length 320.0;
           }
         ~box_sizing:Style.Box_sizing.Border_box ())
      [| node1; node2; node7 |]
    |> Result.get_ok
  in

  (* Compute layout *)
  let _ =
    compute_layout_with_measure tree node0
      {
        width = Available_space.Max_content;
        height = Available_space.Max_content;
      }
      Gentest_helpers.test_measure_function
    |> Result.get_ok
  in

  (* Print tree for debugging *)
  Printf.printf "\nComputed tree:\n";
  print_tree tree node0;
  Printf.printf "\n";

  (* Verify layout *)
  let layout_result = layout tree node1 |> Result.get_ok in
  assert_eq ~msg:"width of node1" 213.0 (Layout.size layout_result).width;
  assert_eq ~msg:"height of node1" 320.0 (Layout.size layout_result).height;
  assert_eq ~msg:"x of node1" 0.0 (Layout.location layout_result).x;
  assert_eq ~msg:"y of node1" 0.0 (Layout.location layout_result).y;
  let layout_result = layout tree node3 |> Result.get_ok in
  assert_eq ~msg:"width of node3" 214.0 (Layout.size layout_result).width;
  assert_eq ~msg:"height of node3" 107.0 (Layout.size layout_result).height;
  assert_eq ~msg:"x of node3" 0.0 (Layout.location layout_result).x;
  assert_eq ~msg:"y of node3" 0.0 (Layout.location layout_result).y;
  let layout_result = layout tree node5 |> Result.get_ok in
  assert_eq ~msg:"width of node5" 214.0 (Layout.size layout_result).width;
  assert_eq ~msg:"height of node5" 106.0 (Layout.size layout_result).height;
  assert_eq ~msg:"x of node5" 0.0 (Layout.location layout_result).x;
  assert_eq ~msg:"y of node5" 0.0 (Layout.location layout_result).y;
  let layout_result = layout tree node4 |> Result.get_ok in
  assert_eq ~msg:"width of node4" 214.0 (Layout.size layout_result).width;
  assert_eq ~msg:"height of node4" 106.0 (Layout.size layout_result).height;
  assert_eq ~msg:"x of node4" 0.0 (Layout.location layout_result).x;
  assert_eq ~msg:"y of node4" 107.0 (Layout.location layout_result).y;
  let layout_result = layout tree node6 |> Result.get_ok in
  assert_eq ~msg:"width of node6" 214.0 (Layout.size layout_result).width;
  assert_eq ~msg:"height of node6" 107.0 (Layout.size layout_result).height;
  assert_eq ~msg:"x of node6" 0.0 (Layout.location layout_result).x;
  assert_eq ~msg:"y of node6" 213.0 (Layout.location layout_result).y;
  let layout_result = layout tree node2 |> Result.get_ok in
  assert_eq ~msg:"width of node2" 214.0 (Layout.size layout_result).width;
  assert_eq ~msg:"height of node2" 320.0 (Layout.size layout_result).height;
  assert_eq ~msg:"x of node2" 213.0 (Layout.location layout_result).x;
  assert_eq ~msg:"y of node2" 0.0 (Layout.location layout_result).y;
  let layout_result = layout tree node7 |> Result.get_ok in
  assert_eq ~msg:"width of node7" 213.0 (Layout.size layout_result).width;
  assert_eq ~msg:"height of node7" 320.0 (Layout.size layout_result).height;
  assert_eq ~msg:"x of node7" 427.0 (Layout.location layout_result).x;
  assert_eq ~msg:"y of node7" 0.0 (Layout.location layout_result).y;
  let layout_result = layout tree node0 |> Result.get_ok in
  assert_eq ~msg:"width of node0" 640.0 (Layout.size layout_result).width;
  assert_eq ~msg:"height of node0" 320.0 (Layout.size layout_result).height;
  assert_eq ~msg:"x of node0" 0.0 (Layout.location layout_result).x;
  assert_eq ~msg:"y of node0" 0.0 (Layout.location layout_result).y;
  ()

let test_flex_rounding_inner_node_controversy_combined_content_box () =
  (* Setup test helpers *)
  let assert_eq ~msg expected actual =
    Windtrap.equal ~msg (Windtrap.Testable.float 0.001) expected actual
  in

  let tree = Gentest_helpers.new_test_tree () in

  (* Create nodes *)
  let node1 =
    new_leaf tree
      (Style.make ~flex_grow:1.0
         ~size:
           {
             width = Style.Dimension.auto;
             height = Style.Dimension.percent 1.0;
           }
         ~box_sizing:Style.Box_sizing.Content_box ())
    |> Result.get_ok
  in
  let node3 =
    new_leaf tree
      (Style.make ~flex_grow:1.0
         ~size:
           {
             width = Style.Dimension.percent 1.0;
             height = Style.Dimension.auto;
           }
         ~box_sizing:Style.Box_sizing.Content_box ())
    |> Result.get_ok
  in
  let node5 =
    new_leaf tree
      (Style.make ~flex_grow:1.0
         ~size:
           {
             width = Style.Dimension.percent 1.0;
             height = Style.Dimension.auto;
           }
         ~box_sizing:Style.Box_sizing.Content_box ())
    |> Result.get_ok
  in
  let node4 =
    new_with_children tree
      (Style.make ~flex_direction:Style.Flex_direction.Column ~flex_grow:1.0
         ~size:
           {
             width = Style.Dimension.percent 1.0;
             height = Style.Dimension.auto;
           }
         ~box_sizing:Style.Box_sizing.Content_box ())
      [| node5 |]
    |> Result.get_ok
  in
  let node6 =
    new_leaf tree
      (Style.make ~flex_grow:1.0
         ~size:
           {
             width = Style.Dimension.percent 1.0;
             height = Style.Dimension.auto;
           }
         ~box_sizing:Style.Box_sizing.Content_box ())
    |> Result.get_ok
  in
  let node2 =
    new_with_children tree
      (Style.make ~flex_direction:Style.Flex_direction.Column ~flex_grow:1.0
         ~size:
           {
             width = Style.Dimension.auto;
             height = Style.Dimension.percent 1.0;
           }
         ~box_sizing:Style.Box_sizing.Content_box ())
      [| node3; node4; node6 |]
    |> Result.get_ok
  in
  let node7 =
    new_leaf tree
      (Style.make ~flex_grow:1.0
         ~size:
           {
             width = Style.Dimension.auto;
             height = Style.Dimension.percent 1.0;
           }
         ~box_sizing:Style.Box_sizing.Content_box ())
    |> Result.get_ok
  in
  let node0 =
    new_with_children tree
      (Style.make ~flex_direction:Style.Flex_direction.Row
         ~size:
           {
             width = Style.Dimension.length 640.0;
             height = Style.Dimension.length 320.0;
           }
         ~box_sizing:Style.Box_sizing.Content_box ())
      [| node1; node2; node7 |]
    |> Result.get_ok
  in

  (* Compute layout *)
  let _ =
    compute_layout_with_measure tree node0
      {
        width = Available_space.Max_content;
        height = Available_space.Max_content;
      }
      Gentest_helpers.test_measure_function
    |> Result.get_ok
  in

  (* Print tree for debugging *)
  Printf.printf "\nComputed tree:\n";
  print_tree tree node0;
  Printf.printf "\n";

  (* Verify layout *)
  let layout_result = layout tree node1 |> Result.get_ok in
  assert_eq ~msg:"width of node1" 213.0 (Layout.size layout_result).width;
  assert_eq ~msg:"height of node1" 320.0 (Layout.size layout_result).height;
  assert_eq ~msg:"x of node1" 0.0 (Layout.location layout_result).x;
  assert_eq ~msg:"y of node1" 0.0 (Layout.location layout_result).y;
  let layout_result = layout tree node3 |> Result.get_ok in
  assert_eq ~msg:"width of node3" 214.0 (Layout.size layout_result).width;
  assert_eq ~msg:"height of node3" 107.0 (Layout.size layout_result).height;
  assert_eq ~msg:"x of node3" 0.0 (Layout.location layout_result).x;
  assert_eq ~msg:"y of node3" 0.0 (Layout.location layout_result).y;
  let layout_result = layout tree node5 |> Result.get_ok in
  assert_eq ~msg:"width of node5" 214.0 (Layout.size layout_result).width;
  assert_eq ~msg:"height of node5" 106.0 (Layout.size layout_result).height;
  assert_eq ~msg:"x of node5" 0.0 (Layout.location layout_result).x;
  assert_eq ~msg:"y of node5" 0.0 (Layout.location layout_result).y;
  let layout_result = layout tree node4 |> Result.get_ok in
  assert_eq ~msg:"width of node4" 214.0 (Layout.size layout_result).width;
  assert_eq ~msg:"height of node4" 106.0 (Layout.size layout_result).height;
  assert_eq ~msg:"x of node4" 0.0 (Layout.location layout_result).x;
  assert_eq ~msg:"y of node4" 107.0 (Layout.location layout_result).y;
  let layout_result = layout tree node6 |> Result.get_ok in
  assert_eq ~msg:"width of node6" 214.0 (Layout.size layout_result).width;
  assert_eq ~msg:"height of node6" 107.0 (Layout.size layout_result).height;
  assert_eq ~msg:"x of node6" 0.0 (Layout.location layout_result).x;
  assert_eq ~msg:"y of node6" 213.0 (Layout.location layout_result).y;
  let layout_result = layout tree node2 |> Result.get_ok in
  assert_eq ~msg:"width of node2" 214.0 (Layout.size layout_result).width;
  assert_eq ~msg:"height of node2" 320.0 (Layout.size layout_result).height;
  assert_eq ~msg:"x of node2" 213.0 (Layout.location layout_result).x;
  assert_eq ~msg:"y of node2" 0.0 (Layout.location layout_result).y;
  let layout_result = layout tree node7 |> Result.get_ok in
  assert_eq ~msg:"width of node7" 213.0 (Layout.size layout_result).width;
  assert_eq ~msg:"height of node7" 320.0 (Layout.size layout_result).height;
  assert_eq ~msg:"x of node7" 427.0 (Layout.location layout_result).x;
  assert_eq ~msg:"y of node7" 0.0 (Layout.location layout_result).y;
  let layout_result = layout tree node0 |> Result.get_ok in
  assert_eq ~msg:"width of node0" 640.0 (Layout.size layout_result).width;
  assert_eq ~msg:"height of node0" 320.0 (Layout.size layout_result).height;
  assert_eq ~msg:"x of node0" 0.0 (Layout.location layout_result).x;
  assert_eq ~msg:"y of node0" 0.0 (Layout.location layout_result).y;
  ()

(* Export tests for aggregation *)
let tests =
  [
    Windtrap.test "rounding_inner_node_controversy_combined (border-box)"
      test_flex_rounding_inner_node_controversy_combined_border_box;
    Windtrap.test "rounding_inner_node_controversy_combined (content-box)"
      test_flex_rounding_inner_node_controversy_combined_content_box;
  ]
