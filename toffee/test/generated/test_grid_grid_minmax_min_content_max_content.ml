...
(* Generated test for grid_minmax_min_content_max_content in grid layout *)
...
let test_grid_minmax_min_content_max_content_border_box () =
  (* Setup test helpers *)
  ...
  (* Create nodes *)
  let node = Toffee.new_leaf tree ({ Toffee.Style.default with display = Toffee.Style.Grid; grid_template_columns = [Toffee.Style.Grid.Single { min = Toffee.Style.Grid.Min_content; max = Toffee.Style.Grid.Max_content }]; grid_template_rows = [Toffee.Style.Grid.Single { min = Toffee.Style.Grid.Length (40.0); max = Toffee.Style.Grid.Length (40.0) }] }) in
...
let test_grid_minmax_min_content_max_content_content_box () =
  (* Setup test helpers *)
  let assert_eq ~msg expected actual =
    let open Alcotest in
    check (float 0.001) msg expected actual
...
let () =
  let open Alcotest in
  run "Toffee grid_minmax_min_content_max_content Test" [
    "grid_grid_minmax_min_content_max_content", [
      test_case "grid_minmax_min_content_max_content (border-box)" `Quick test_grid_minmax_min_content_max_content_border_box;
      test_case "grid_minmax_min_content_max_content (content-box)" `Quick test_grid_minmax_min_content_max_content_content_box;
    ];
  ]
