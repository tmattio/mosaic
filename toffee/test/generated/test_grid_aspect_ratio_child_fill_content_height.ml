...
let test_grid_aspect_ratio_child_fill_content_height_border_box measure_function
    () =
  (* Setup test helpers *)
  ...
  (* Create nodes *)
  let node1 =
    new_leaf tree (Style.make ~aspect_ratio:0.5 ()) |> Result.get_ok
  in
  let _ =
    set_node_context tree node1 (Some (MeasureFunction.Text "HHHH"))
    |> Result.get_ok
  in
  let node2 =
    new_leaf tree
...
let test_grid_aspect_ratio_child_fill_content_height_content_box
  ...
  let node1 =
    new_leaf tree
      (Style.make ~aspect_ratio:0.5 ~box_sizing:Style.Box_sizing.Content_box ())
    |> Result.get_ok
  in
  let _ =
    set_node_context tree node1 (Some (MeasureFunction.Text "HHHH"))
...
let tests =
  let open Alcotest in
  [
    test_case "aspect_ratio_child_fill_content_height (border-box)" `Quick
      (fun () ->
        test_grid_aspect_ratio_child_fill_content_height_border_box
          measure_function ());
    test_case "aspect_ratio_child_fill_content_height (content-box)" `Quick
      (fun () ->
        test_grid_aspect_ratio_child_fill_content_height_content_box
          measure_function ());
  ]
