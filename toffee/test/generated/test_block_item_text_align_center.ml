  ...
  let node0 =
    new_with_children tree
      (Style.make ~display:Style.Display.Block
         ...
         ~text_align:Style.Text_align.Legacy_center
         ~box_sizing:Style.Box_sizing.Border_box ())
      [| node1; node2 |]
    |> Result.get_ok
  in
...
let tests =
  let open Alcotest in
  [
    test_case "item_text_align_center (border-box)" `Quick
      test_block_item_text_align_center_border_box;
    test_case "item_text_align_center (content-box)" `Quick
      test_block_item_text_align_center_content_box;
  ]
