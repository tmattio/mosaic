...
let test_flex_measure_child_constraint_border_box measure_function () =
  (* Setup test helpers *)
...
let test_flex_measure_child_constraint_content_box measure_function () =
  (* Setup test helpers *)
  let assert_eq ~msg expected actual =
    let open Alcotest in
    check (float 0.001) msg expected actual
...
let tests =
  let open Alcotest in
  [
    test_case "measure_child_constraint (border-box)" `Quick (fun () ->
        test_flex_measure_child_constraint_border_box measure_function ());
    test_case "measure_child_constraint (content-box)" `Quick (fun () ->
        test_flex_measure_child_constraint_content_box measure_function ());
  ]
