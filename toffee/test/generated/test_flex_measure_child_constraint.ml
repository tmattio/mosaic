...
(* Generated test for measure_child_constraint in flex layout *)
...
let test_measure_child_constraint_border_box () =
  (* Setup test helpers *)
  ...
  (* Create nodes *)
  let node = Toffee.new_leaf tree ({ Toffee.Style.default with size = { width = Toffee.Style.Dimension.length 50.0; height = Toffee.Style.Dimension.auto } }) in
...
let test_measure_child_constraint_content_box () =
  (* Setup test helpers *)
  let assert_eq ~msg expected actual =
    let open Alcotest in
    check (float 0.001) msg expected actual
  ...
  (* Create nodes *)
  let node = Toffee.new_leaf tree ({ Toffee.Style.default with size = { width = Toffee.Style.Dimension.length 50.0; height = Toffee.Style.Dimension.auto }; box_sizing = Toffee.Style.Content_box }) in
...
let () =
  let open Alcotest in
  run "Toffee measure_child_constraint Test" [
    "flex_measure_child_constraint", [
      test_case "measure_child_constraint (border-box)" `Quick test_measure_child_constraint_border_box;
      test_case "measure_child_constraint (content-box)" `Quick test_measure_child_constraint_content_box;
    ];
  ]
