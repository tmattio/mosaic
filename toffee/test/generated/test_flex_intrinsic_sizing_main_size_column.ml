...
let test_flex_intrinsic_sizing_main_size_column_border_box measure_function () =
  (* Setup test helpers *)
  ...
  in
  let _ =
    set_node_context tree node (Some (MeasureFunction.Text_vertical "HH​HH"))
    |> Result.get_ok
  in
...
let test_flex_intrinsic_sizing_main_size_column_content_box measure_function ()
    =
  (* Setup test helpers *)
  let assert_eq ~msg expected actual =
    let open Alcotest in
...
let tests =
  let open Alcotest in
  [
    test_case "intrinsic_sizing_main_size_column (border-box)" `Quick (fun () ->
        test_flex_intrinsic_sizing_main_size_column_border_box measure_function
          ());
    test_case "intrinsic_sizing_main_size_column (content-box)" `Quick
      (fun () ->
        test_flex_intrinsic_sizing_main_size_column_content_box measure_function
          ());
  ]
