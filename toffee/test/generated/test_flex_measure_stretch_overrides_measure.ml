...
let test_flex_measure_stretch_overrides_measure_border_box measure_function () =
  ...
  let node0 =
    new_leaf tree
      (Style.make ~flex_grow:1.0 ~flex_basis:(Style.Dimension.length 5.0) ())
    |> Result.get_ok
  in
  ...
  in
  let _ =
    set_node_context tree node1 (Some (MeasureFunction.Text "H"))
    |> Result.get_ok
  in
  ...
  let node0 =
    new_leaf tree
      (Style.make ~flex_grow:1.0
         ~flex_basis:(Style.Dimension.length 5.0)
         ~box_sizing:Style.Box_sizing.Content_box ())
    |> Result.get_ok
  in
  let _ = add_child tree node node0 |> Result.get_ok in
