...
let test_flex_gap_column_gap_determines_parent_width_border_box () =
  (* Setup test helpers *)
  ...
  let node =
    new_leaf tree
      (Style.make ~flex_direction:Style.Flex_direction.Row ~align_items:Stretch
         ~size:
           {
...
let test_flex_gap_column_gap_determines_parent_width_content_box () =
  (* Setup test helpers *)
  let assert_eq ~msg expected actual =
    let open Alcotest in
    check (float 0.001) msg expected actual
...
let tests =
  let open Alcotest in
  [
    test_case "gap_column_gap_determines_parent_width (border-box)" `Quick
      test_flex_gap_column_gap_determines_parent_width_border_box;
    test_case "gap_column_gap_determines_parent_width (content-box)" `Quick
      test_flex_gap_column_gap_determines_parent_width_content_box;
  ]
