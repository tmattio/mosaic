(* Generated test for grid_percent_items_nested_moderate in grid layout *)
(* Do not edit this file directly. It is generated by gentest.js *)

open Toffee

let test_grid_percent_items_nested_moderate_border_box () =
  (* Setup test helpers *)
  let assert_eq ~msg expected actual =
    let open Alcotest in
    check (float 0.1) msg expected actual
  in

  let tree = Gentest_helpers.new_test_tree () in
  let tree = disable_rounding tree in

  (* Create nodes *)
  let node2 =
    new_leaf tree
      (Style.make
         ~size:
           {
             width = Style.Dimension.percent 0.45;
             height = Style.Dimension.auto;
           }
         ~margin:
           {
             left = Style.Length_percentage_auto.percent 0.05;
             right = Style.Length_percentage_auto.percent 0.05;
             top = Style.Length_percentage_auto.percent 0.05;
             bottom = Style.Length_percentage_auto.percent 0.05;
           }
         ~padding:
           {
             left = Style.Length_percentage.length 3.0;
             right = Style.Length_percentage.length 3.0;
             top = Style.Length_percentage.length 3.0;
             bottom = Style.Length_percentage.length 3.0;
           }
         ~box_sizing:Style.Box_sizing.Border_box ())
    |> Result.get_ok
  in
  let node1 =
    new_with_children tree
      (Style.make ~display:Style.Display.Grid
         ~size:
           {
             width = Style.Dimension.percent 0.5;
             height = Style.Dimension.auto;
           }
         ~margin:
           {
             left = Style.Length_percentage_auto.length 5.0;
             right = Style.Length_percentage_auto.length 5.0;
             top = Style.Length_percentage_auto.length 5.0;
             bottom = Style.Length_percentage_auto.length 5.0;
           }
         ~padding:
           {
             left = Style.Length_percentage.percent 0.03;
             right = Style.Length_percentage.percent 0.03;
             top = Style.Length_percentage.percent 0.03;
             bottom = Style.Length_percentage.percent 0.03;
           }
         ~box_sizing:Style.Box_sizing.Border_box ())
      [| node2 |]
    |> Result.get_ok
  in
  let node0 =
    new_with_children tree
      (Style.make ~display:Style.Display.Grid
         ~size:
           {
             width = Style.Dimension.length 200.0;
             height = Style.Dimension.auto;
           }
         ~padding:
           {
             left = Style.Length_percentage.length 3.0;
             right = Style.Length_percentage.length 3.0;
             top = Style.Length_percentage.length 3.0;
             bottom = Style.Length_percentage.length 3.0;
           }
         ~box_sizing:Style.Box_sizing.Border_box ())
      [| node1 |]
    |> Result.get_ok
  in

  (* Compute layout *)
  let _ =
    compute_layout_with_measure tree node0
      {
        width = Available_space.Max_content;
        height = Available_space.Max_content;
      }
      Gentest_helpers.test_measure_function
    |> Result.get_ok
  in

  (* Print tree for debugging *)
  Printf.printf "\nComputed tree:\n";
  print_tree tree node0;
  Printf.printf "\n";

  (* Verify layout *)
  let layout_result = layout tree node2 |> Result.get_ok in
  assert_eq ~msg:"width of node2" 38.40625 (Layout.size layout_result).width;
  assert_eq ~msg:"height of node2" 6.0 (Layout.size layout_result).height;
  assert_eq ~msg:"x of node2" 10.078125 (Layout.location layout_result).x;
  assert_eq ~msg:"y of node2" 10.078125 (Layout.location layout_result).y;
  let layout_result = layout tree node1 |> Result.get_ok in
  assert_eq ~msg:"width of node1" 97.0 (Layout.size layout_result).width;
  assert_eq ~msg:"height of node1" 26.15625 (Layout.size layout_result).height;
  assert_eq ~msg:"x of node1" 8.0 (Layout.location layout_result).x;
  assert_eq ~msg:"y of node1" 8.0 (Layout.location layout_result).y;
  let layout_result = layout tree node0 |> Result.get_ok in
  assert_eq ~msg:"width of node0" 200.0 (Layout.size layout_result).width;
  assert_eq ~msg:"height of node0" 42.15625 (Layout.size layout_result).height;
  assert_eq ~msg:"x of node0" 0.0 (Layout.location layout_result).x;
  assert_eq ~msg:"y of node0" 0.0 (Layout.location layout_result).y;
  ()

let test_grid_percent_items_nested_moderate_content_box () =
  (* Setup test helpers *)
  let assert_eq ~msg expected actual =
    let open Alcotest in
    check (float 0.1) msg expected actual
  in

  let tree = Gentest_helpers.new_test_tree () in
  let tree = disable_rounding tree in

  (* Create nodes *)
  let node2 =
    new_leaf tree
      (Style.make
         ~size:
           {
             width = Style.Dimension.percent 0.45;
             height = Style.Dimension.auto;
           }
         ~margin:
           {
             left = Style.Length_percentage_auto.percent 0.05;
             right = Style.Length_percentage_auto.percent 0.05;
             top = Style.Length_percentage_auto.percent 0.05;
             bottom = Style.Length_percentage_auto.percent 0.05;
           }
         ~padding:
           {
             left = Style.Length_percentage.length 3.0;
             right = Style.Length_percentage.length 3.0;
             top = Style.Length_percentage.length 3.0;
             bottom = Style.Length_percentage.length 3.0;
           }
         ~box_sizing:Style.Box_sizing.Content_box ())
    |> Result.get_ok
  in
  let node1 =
    new_with_children tree
      (Style.make ~display:Style.Display.Grid
         ~size:
           {
             width = Style.Dimension.percent 0.5;
             height = Style.Dimension.auto;
           }
         ~margin:
           {
             left = Style.Length_percentage_auto.length 5.0;
             right = Style.Length_percentage_auto.length 5.0;
             top = Style.Length_percentage_auto.length 5.0;
             bottom = Style.Length_percentage_auto.length 5.0;
           }
         ~padding:
           {
             left = Style.Length_percentage.percent 0.03;
             right = Style.Length_percentage.percent 0.03;
             top = Style.Length_percentage.percent 0.03;
             bottom = Style.Length_percentage.percent 0.03;
           }
         ~box_sizing:Style.Box_sizing.Content_box ())
      [| node2 |]
    |> Result.get_ok
  in
  let node0 =
    new_with_children tree
      (Style.make ~display:Style.Display.Grid
         ~size:
           {
             width = Style.Dimension.length 200.0;
             height = Style.Dimension.auto;
           }
         ~padding:
           {
             left = Style.Length_percentage.length 3.0;
             right = Style.Length_percentage.length 3.0;
             top = Style.Length_percentage.length 3.0;
             bottom = Style.Length_percentage.length 3.0;
           }
         ~box_sizing:Style.Box_sizing.Content_box ())
      [| node1 |]
    |> Result.get_ok
  in

  (* Compute layout *)
  let _ =
    compute_layout_with_measure tree node0
      {
        width = Available_space.Max_content;
        height = Available_space.Max_content;
      }
      Gentest_helpers.test_measure_function
    |> Result.get_ok
  in

  (* Print tree for debugging *)
  Printf.printf "\nComputed tree:\n";
  print_tree tree node0;
  Printf.printf "\n";

  (* Verify layout *)
  let layout_result = layout tree node2 |> Result.get_ok in
  assert_eq ~msg:"width of node2" 51.0 (Layout.size layout_result).width;
  assert_eq ~msg:"height of node2" 6.0 (Layout.size layout_result).height;
  assert_eq ~msg:"x of node2" 11.0 (Layout.location layout_result).x;
  assert_eq ~msg:"y of node2" 11.0 (Layout.location layout_result).y;
  let layout_result = layout tree node1 |> Result.get_ok in
  assert_eq ~msg:"width of node1" 112.0 (Layout.size layout_result).width;
  assert_eq ~msg:"height of node1" 28.0 (Layout.size layout_result).height;
  assert_eq ~msg:"x of node1" 8.0 (Layout.location layout_result).x;
  assert_eq ~msg:"y of node1" 8.0 (Layout.location layout_result).y;
  let layout_result = layout tree node0 |> Result.get_ok in
  assert_eq ~msg:"width of node0" 206.0 (Layout.size layout_result).width;
  assert_eq ~msg:"height of node0" 44.0 (Layout.size layout_result).height;
  assert_eq ~msg:"x of node0" 0.0 (Layout.location layout_result).x;
  assert_eq ~msg:"y of node0" 0.0 (Layout.location layout_result).y;
  ()

(* Export tests for aggregation *)
let tests =
  let open Alcotest in
  [
    test_case "percent_items_nested_moderate (border-box)" `Quick
      test_grid_percent_items_nested_moderate_border_box;
    test_case "percent_items_nested_moderate (content-box)" `Quick
      test_grid_percent_items_nested_moderate_content_box;
  ]
